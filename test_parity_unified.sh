#!/bin/bash
# =============================================================================
# UNIFIED INK PARITY TEST HARNESS - All 3 Environments with Per-Symbol Diffs
# =============================================================================
# Tests 100% look&feel parity across 3 environments:
#   1. deno        - Reference TypeScript runtime (npm:ink@5.0.1)
#   2. runts dev   - HIR runtime (QuickJS/HIR interpreter with hot-reload)
#   3. runts build - In-memory transpile + Rust compilation
#
# Features:
# - Unified single script for all parity testing
# - Per-symbol diff results
# - Output tracing to files
# - Detailed failure analysis
# - High test coverage
#
# Usage: ./test_parity_unified.sh [--quick] [--examples name...] [--verbose]
# =============================================================================
...
RESULTS_DIR="$SCRIPT_DIR/.parity_results"
LOG_DIR="$SCRIPT_DIR/.parity_logs"
DIFF_DIR="$SCRIPT_DIR/.parity_diffs"
SYMBOL_DIR="$SCRIPT_DIR/.parity_symbols"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            echo "Usage: $0 [--quick] [--examples name...] [--verbose]"
            exit 0
            ;;
...
# Cleanup on exit
cleanup() {
    rm -rf "$RESULTS_DIR" "$LOG_DIR" "$DIFF_DIR" "$SYMBOL_DIR" 2>/dev/null || true
}
trap cleanup EXIT

mkdir -p "$RESULTS_DIR" "$LOG_DIR" "$DIFF_DIR" "$SYMBOL_DIR"
...
extract_symbols() {
    ...
    grep -vE '^(ink|react|use|import|from|export|function|const|let|var|default|return|if|else|for|while|new|typeof)$' | \
    sort -u
}
...
calc_symbol_similarity() {
    ...
    local count1=$(echo "$sym1" | wc -l | tr -d ' ')
    local count2=$(echo "$sym2" | wc -l | tr -d ' ')
    
    [[ "$count1" -eq 0 ]] && [[ "$count2" -eq 0 ]] && { echo "100"; return; }
    [[ "$count1" -eq 0 ]] || [[ "$count2" -eq 0 ]] && { echo "0"; return; }
    
    local matching=$(comm -12 <(echo "$sym1") <(echo "$sym2") 2>/dev/null | wc -l | tr -d ' ')
    local max=$((count1 > count2 ? count1 : count2))
    [[ $max -eq 0 ]] && max=1
    
    echo $((matching * 100 / max))
}
...
generate_symbol_diff() {
    ...
    local sym2=$(extract_symbols "$file2")
    {
        ...
        echo ""
        echo "=== Common symbols ==="
        comm -12 <(echo "$sym1") <(echo "$sym2") 2>/dev/null || true
        echo ""
        echo "=== Only in file1 ==="
...
generate_content_diff() {
    local file1=$1; local file2=$2; local out_file=$3
    local cont1=$(extract_content "$file1")
    local cont2=$(extract_content "$file2")
    {
        echo "=== Content in file1 ==="
        echo "$cont1"
        echo ""
        echo "=== Content in file2 ==="
        echo "$cont2"
        echo ""
        echo "=== Common content ==="
        comm -12 <(echo "$cont1") <(echo "$cont2") 2>/dev/null || true
        echo ""
        echo "=== Only in file1 ==="
        comm -23 <(echo "$cont1") <(echo "$cont2") 2>/dev/null || true
        echo ""
        echo "=== Only in file2 ==="
        comm -13 <(echo "$cont1") <(echo "$cont2") 2>/dev/null || true
    } > "$out_file"
}
...
test_example() {
    ...
    local example_symbols="$SYMBOL_DIR/$name"
    
    mkdir -p "$example_results" "$example_diffs" "$example_symbols"
    ...
    local dh_sym_sim=$(calc_symbol_similarity "$deno_file" "$hir_file")
    echo -n "  └─ similarity: D-H:${dh_sim}% sym:${dh_sym_sim}%"
    
    local passed=true
    local reason=""
    
    # Pass threshold: 60%
    [[ $dh_sim -lt 60 ]] && { passed=false; reason="${reason}D-H: ${dh_sim}%"; }
    
    if [[ "$QUICK_MODE" != "true" ]]; then
        local dc_sim=$(calc_similarity "$deno_file" "$compile_file")
        local hc_sim=$(calc_similarity "$hir_file" "$compile_file")
        local dc_sym_sim=$(calc_symbol_similarity "$deno_file" "$compile_file")
        local hc_sym_sim=$(calc_symbol_similarity "$hir_file" "$compile_file")
        echo " D-C:${dc_sim}% H-C:${hc_sim}%"
        echo "     symbol sims: D-C:${dc_sym_sim}% H-C:${hc_sym_sim}%"
        ...
        # Generate symbol diffs
        generate_symbol_diff "$deno_file" "$hir_file" "$example_symbols/deno_vs_hir_symbols.diff"
        generate_symbol_diff "$deno_file" "$compile_file" "$example_symbols/deno_vs_compile_symbols.diff"
        generate_symbol_diff "$hir_file" "$compile_file" "$example_symbols/hir_vs_compile_symbols.diff"
        
        # Generate content diffs
        generate_content_diff "$deno_file" "$hir_file" "$example_symbols/deno_vs_hir_content.diff"
        generate_content_diff "$deno_file" "$compile_file" "$example_symbols/deno_vs_compile_content.diff"
    else
        echo ""
        generate_diff "$deno_file" "$hir_file" "$example_diffs/deno_vs_hir.diff"
        generate_symbol_diff "$deno_file" "$hir_file" "$example_symbols/deno_vs_hir_symbols.diff"
    ...
    echo "$name|$deno_result|$hir_result|$compile_result|$dh_sim|${dh_sym_sim}|${status}|${reason}" >> "$RESULTS_DIR/summary.csv"
...
run_tests() {
    # Create summary CSV
    echo "# Example|Deno Result|HIR Result|Compile Result|D-H Sim%|D-H Sym Sim%|Status|Reason" > "$RESULTS_DIR/summary.csv"
    ...
    echo ""
    echo -e "${BOLD}╔════════════════════════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}║${NC}  ${CYAN}UNIFIED INK PARITY TEST HARNESS${NC}                                             ${BOLD}║${NC}"
    echo -e "${BOLD}╠════════════════════════════════════════════════════════════════════════════════════╣${NC}"
    echo -e "${BOLD}║${NC}  Environments: ${GREEN}deno${NC} | ${GREEN}runts dev (HIR)${NC} | ${GREEN}runts build (compile)${NC}                    ${BOLD}║${NC}"
    if [[ "$QUICK_MODE" == "true" ]]; then
        MODE_TEXT="QUICK (deno + HIR only)"
    else
        MODE_TEXT="FULL (all 3 environments)"
    fi
    echo -e "${BOLD}║${NC}  Mode: ${YELLOW}${MODE_TEXT}${NC}                                                           ${BOLD}║${NC}"
    echo -e "${BOLD}╚════════════════════════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "Results: ${CYAN}$RESULTS_DIR${NC}"
    echo -e "Diffs:   ${CYAN}$DIFF_DIR${NC}"
    echo -e "Symbols: ${CYAN}$SYMBOL_DIR${NC}"
    echo ""

...
(async () => {
  ...
  if (fs.readFileSync(path, 'utf8') !== 'abc') throw new Error('filehandle string write mismatch');
  for (const value of [123, {}, null, undefined, true]) {
    try { await handle.write(value, 0, 1); throw new Error('accepted invalid filehandle write'); }

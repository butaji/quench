...
const __nodeURLResolveInput = (input, base) => {
  // prettier-ignore
  let value = (/^file:[a-z][|:][\\/]/i.test(String(input).trim().replace(/[\t\n\r]/g, "")) || (/^\/{0,2}[a-z][|:][\\/]/i.test(String(input).trim().replace(/[\t\n\r]/g, "")) && /^file:/i.test(base || ""))) ? `file:///${String(input).trim().replace(/[\t\n\r]/g, "").replace(/^file:/i, "").replace(/^\/{0,2}(?=[a-z][|:])/i, "").replace(/^([a-z])\|/i, "$1:").replace(/\\/g, "/")}` : /^(?:\\\\|\/\\)/.test(String(input).trim()) && /^file:/i.test(base || "") ? String(input).trim().replace(/^(?:\\\\|\/\\)/, "//").replace(/\\/g, "/") : String(input).trim().replace(/[\t\n\r]/g, "");
  // prettier-ignore
  if (base && (value === "" ? (value = String(base).trim().replace(/[\t\n\r]/g, "")) : /^(?:https?|ftp):\/\//i.test(base) && (value = value.replace(/^[^?#]*/, (path) => path.replace(/\\/g, "/")))));
  // prettier-ignore
  if (base && (!/^[a-z][a-z0-9+.-]*:/.test(value) || (/^https?:[^/]/.test(value) && value.slice(0, value.indexOf(":")) === String(base).slice(0, String(base).indexOf(":"))) || (/^(?:https?):\/(?:[^/]|$)/.test(value) && value.slice(0, value.indexOf(":")) === String(base).slice(0, String(base).indexOf(":"))))) { const baseUrl = new globalThis.__nodeURL(base); value = value.replace(/^(?:https?|ftp):/, "");
...
globalThis.__nodeURL = class NodeURL {
  ...
  get href() {
    ...
    // prettier-ignore
    const credentials = this.username || this.password ? `${this.username || ""}${this.password ? `:${this.password}` : ""}@` : "" , prefix = this.protocol === "file:" ? `file://${this.host}` : this.origin === "null" ? this.protocol + (this._hasAuthorityDelimiter ? `//${credentials}${this.host}` : "") : this.origin.replace(/^(\w+:\/\/)/, `$1${credentials}`);

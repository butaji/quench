...
const __nodeBufferByteLength = (value, encoding) => {
  ...
  const error = new TypeError(
    `The "string" argument must be of type string or an instance of Buffer or ArrayBuffer.${
      __nodeBufferFromReceived(value)
    }`,
  );
...
class NodeBuffer extends Uint8Array {
  ...
  static from(value, encoding, length) {
    ...
    if (
      __nodeBufferIsArrayBuffer(value) || value instanceof SharedArrayBuffer
    ) {
  ...
  static isUtf8(value) {
    return __nodeIsValidUtf8(__nodeValidateUtf8Input(value));
  }
  ...
  static byteLength(value, encoding = "utf8") {
    return __nodeBufferByteLength(value, encoding);
  }

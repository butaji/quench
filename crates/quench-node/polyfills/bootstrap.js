...
globalThis.__nodeFs = {
  ...
  linkSync: (existing, link) => globalThis.__quench_fs_link(nodeFsPath(existing), nodeFsPath(link)),
...
globalThis.__nodeFs.link = (existing, link, callback) => { if (typeof callback !== 'function') throw new TypeError('The "callback" argument must be of type function'); queueMicrotask(() => { try { globalThis.__nodeFs.linkSync(existing, link); callback(null); } catch (error) { callback(error); } }); };

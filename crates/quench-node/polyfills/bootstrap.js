...
globalThis.__nodeFs.symlink = (target, link, type, callback) => { if (typeof type === 'function') callback = type; if (typeof callback !== 'function') throw new TypeError('The "callback" argument must be of type function'); if (typeof target !== 'string' && !(target instanceof Uint8Array) || typeof link !== 'string' && !(link instanceof Uint8Array)) { const error = new TypeError('The "target" and "path" arguments must be strings or Buffer'); error.code = 'ERR_INVALID_ARG_TYPE'; throw error; } const source = nodePathValue(target); const destination = nodeFsPath(link); queueMicrotask(() => { try { globalThis.__quench_fs_symlink(source, destination); } catch (error) { callback(error); return; } callback(null); }); };

...
class NodeReadable extends NodeEventEmitter {
  static from(iterable) {
    ...
    queueMicrotask(() => { for (const chunk of stream._chunks) stream.emit('data', chunk); stream.emit('end'); });
    return stream;
  }
...
globalThis.__nodeFs = {
  ...
  symlinkSync: (target, link, type) => { if (typeof target !== 'string' && !(target instanceof Uint8Array) || typeof link !== 'string' && !(link instanceof Uint8Array)) { const error = new TypeError('The "target" and "path" arguments must be strings or Buffer'); error.code = 'ERR_INVALID_ARG_TYPE'; throw error; } if (type !== undefined && type !== 'file' && type !== 'dir' && type !== 'junction') { const error = new TypeError('The "type" argument is invalid'); error.code = 'ERR_INVALID_ARG_VALUE'; throw error; } return globalThis.__quench_fs_symlink(nodeFsPath(target), nodeFsPath(link)); },
  linkSync: (existing, link) => { if (typeof existing !== 'string' && !(existing instanceof Uint8Array) || typeof link !== 'string' && !(link instanceof Uint8Array)) { const error = new TypeError('The "path" argument must be of type string or an instance of Buffer or URL'); error.code = 'ERR_INVALID_ARG_TYPE'; throw error; } return globalThis.__quench_fs_link(nodeFsPath(existing), nodeFsPath(link)); },
  readlinkSync: (value, options) => { const result = globalThis.__quench_fs_readlink(nodeFsPath(value)); const encoding = typeof options === 'string' ? options : options && options.encoding; return encoding === 'buffer' ? NodeBuffer.from(result) : encoding ? NodeBuffer.from(result).toString(encoding) : result; },
};
...
globalThis.__nodeFs.symlink = (target, link, type, callback) => { if (typeof type === 'function') { callback = type; type = undefined; } if (typeof callback !== 'function') throw new TypeError('The "callback" argument must be of type function'); if (typeof target !== 'string' && !(target instanceof Uint8Array) || typeof link !== 'string' && !(link instanceof Uint8Array)) { const error = new TypeError('The "target" and "path" arguments must be strings or Buffer'); error.code = 'ERR_INVALID_ARG_TYPE'; throw error; } if (type !== undefined && type !== 'file' && type !== 'dir' && type !== 'junction') { const error = new TypeError('The "type" argument is invalid'); error.code = 'ERR_INVALID_ARG_VALUE'; throw error; } const source = nodePathValue(target); const destination = nodeFsPath(link); queueMicrotask(() => { try { globalThis.__quench_fs_symlink(source, destination); } catch (error) { callback(error); return; } callback(null); }); };
globalThis.__nodeFs.readlink = (value, options, callback) => { if (typeof options === 'function') { callback = options; options = undefined; } if (typeof callback !== 'function') throw new TypeError('The "callback" argument must be of type function'); const path = nodeFsPath(value); queueMicrotask(() => { try { callback(null, globalThis.__nodeFs.readlinkSync(path, options)); } catch (error) { callback(error); return; } }); };
globalThis.__nodeFs.link = (existing, link, callback) => { if (typeof callback !== 'function') throw new TypeError('The "callback" argument must be of type function'); if (typeof existing !== 'string' && !(existing instanceof Uint8Array) || typeof link !== 'string' && !(link instanceof Uint8Array)) { const error = new TypeError('The "path" argument must be of type string or an instance of Buffer or URL'); error.code = 'ERR_INVALID_ARG_TYPE'; throw error; } queueMicrotask(() => { try { globalThis.__nodeFs.linkSync(existing, link); callback(null); } catch (error) { callback(error); } }); };
...
globalThis.__nodeFs.promises = {
  ...
  mkdtemp: (prefix) => Promise.resolve().then(() => globalThis.__nodeFs.mkdtempSync(prefix)),
  ...
  lstat: (value) => Promise.resolve().then(() => globalThis.__nodeFs.lstatSync(value)),
  link: (existing, link) => Promise.resolve().then(() => globalThis.__nodeFs.linkSync(existing, link)),
};
const __nodePromiseOpen = globalThis.__nodeFs.promises.open;
globalThis.__nodeFs.promises.open = async (...args) => { const handle = await __nodePromiseOpen(...args); handle.write = (buffer, offset, length, position) => Promise.resolve().then(() => { const start = typeof offset === 'object' ? (offset.offset || 0) : (offset || 0); const source = typeof offset === 'object' ? (offset.buffer || offset) : buffer; const size = typeof offset === 'object' ? (offset.length === undefined ? source.length - start : offset.length) : (length === undefined ? source.length - start : length); const at = typeof offset === 'object' ? offset.position : position; return { bytesWritten: globalThis.__nodeFs.writeSync(handle.fd, source, start, size, at === undefined ? null : at), buffer: source }; }); handle.readv = (buffers, position) => Promise.resolve().then(() => ({ bytesRead: globalThis.__nodeFs.readvSync(handle.fd, buffers, position), buffers })); handle.writev = (buffers, position) => Promise.resolve().then(() => ({ bytesWritten: globalThis.__nodeFs.writevSync(handle.fd, buffers, position), buffers })); handle.truncate = (length = 0) => Promise.resolve().then(() => globalThis.__nodeFs.ftruncateSync(handle.fd, length)); handle.stat = () => Promise.resolve().then(() => globalThis.__nodeFs.fstatSync(handle.fd)); handle.sync = () => Promise.resolve().then(() => globalThis.__nodeFs.fsyncSync(handle.fd)); handle.datasync = () => Promise.resolve().then(() => globalThis.__nodeFs.fdatasyncSync(handle.fd)); handle.chmod = (mode) => Promise.resolve().then(() => globalThis.__nodeFs.chmodSync(globalThis.__nodeFdPaths[handle.fd], mode)); handle.readFile = (options) => Promise.resolve().then(() => globalThis.__nodeFs.readFileSync(handle.fd, options)); handle.writeFile = async (data, options) => { if (options && options.signal) await new Promise((resolve) => queueMicrotask(resolve)); if (options && options.signal && options.signal.aborted) { const error = new Error('The operation was aborted'); error.name = 'AbortError'; error.code = 'ABORT_ERR'; throw error; } if (data && data._chunks) data = data._chunks; if (data && typeof data !== 'string' && !(data instanceof Uint8Array) && !(data instanceof ArrayBuffer) && typeof data[Symbol.asyncIterator] === 'function') { const chunks = []; for await (const chunk of data) chunks.push(chunk); data = chunks; } if (data && typeof data !== 'string' && !(data instanceof Uint8Array) && !ArrayBuffer.isView(data) && typeof data[Symbol.iterator] === 'function') { const chunks = []; for (const chunk of data) { if (typeof chunk !== 'string' && !(chunk instanceof Uint8Array) && !ArrayBuffer.isView(chunk)) { const error = new TypeError('The "data" argument must be of type string or an instance of Buffer, TypedArray, or DataView'); error.code = 'ERR_INVALID_ARG_TYPE'; throw error; } chunks.push(chunk); } data = chunks.length === 1 ? chunks[0] : chunks.reduce((all, chunk) => all + (typeof chunk === 'string' ? chunk : NodeBuffer.from(chunk).toString('utf8')), ''); } return globalThis.__nodeFs.writeFileSync(handle.fd, data, typeof options === 'string' ? { encoding: options } : options); }; handle.appendFile = (data, options) => Promise.resolve().then(() => globalThis.__nodeFs.appendFileSync(handle.fd, data, options)); handle.close = () => Promise.resolve().then(() => globalThis.__nodeFs.closeSync(handle.fd)); return handle; };
globalThis.__nodeOs = {
  EOL: '\n',

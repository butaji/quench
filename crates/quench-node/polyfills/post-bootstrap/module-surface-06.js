{
  ...
  const initializeURLPattern = (instance, options) => {
    ...
    for (const property of "origin protocol username password hostname port pathname search hash".split(
      " "
    )) {
          ...
          : property === "search"
            ...
            : property === "hash"
              ? String(value) === ""
                ? ""
                : `#${globalThis
                    .__nodeUrlEncode(String(value).replace(/^#/, ""))
                    .replace(/%2F/gi, "/")
                    .replace(/%5C/gi, "\\")
                    .replace(/%23/gi, "#")
                    .replace(/%3B/gi, ";")
  ...
  const installURLAccessorDescriptors = (prototype) => {
    for (const property of "origin protocol username password host hostname port pathname search searchParams hash".split(
      " "
    )) {
      const accessor =
  ...
  const __nodeURLInspect = (receiver, options = {}) => {
    const params = receiver.searchParams.toString()
      ? `URLSearchParams { '${receiver.searchParams._pairs
  ...
  const setURLHref = Object.getOwnPropertyDescriptor(
    {
      set href(value) {
        ...
        for (const property of "protocol username password host hostname port pathname search hash".split(
          " "
        )) {
  ...
  const reorderURLProperties = (prototype) => {
    for (const property of "toString href origin protocol username password host hostname port pathname search searchParams hash toJSON".split(
      " "
    )) {

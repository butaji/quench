...
// prettier-ignore
globalThis.__quenchNormalizeSpecialUrlInput = (value) => /^(?:https?|ftp|wss?):/i.test(value) && !/^(?:https?|ftp|wss?):\/\//i.test(value) ? value.replace(/^((?:https?|ftp|wss?):)\/?(?!\/)/i, "$1//").replace(/^[^?#]*/, (path) => path.replace(/\\/g, "/")) : value;

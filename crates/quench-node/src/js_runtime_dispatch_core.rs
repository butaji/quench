    ...
    fn dispatch_core(
        ...
        let result = (|| -> Result<Value, VmError> {
            match capability.kind {
                HostCapabilityKind::Custom(CapabilityName::Require) => match arguments.first() {
                ...
                HostCapabilityKind::Custom(CapabilityName::CommonTick) => {
                    let callback = arguments.get(1).cloned().unwrap_or(Value::Undefined);
                    let mut next_tick = vec![callback];
                    next_tick.extend_from_slice(arguments.get(2..).unwrap_or(&[]));
                    crate::modules::process::next_tick(&self.state, &next_tick)
                }

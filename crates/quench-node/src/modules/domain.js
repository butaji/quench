  ...
  const d = {
    ...
    enter() {
      if (!d.disposed) {
        ...
        if (index >= 0) stack = stack.slice(0, index).concat(stack.slice(index + 1));
        stack = stack.concat([d]);
        module.exports._stack = stack;
    ...
    add(x) {
      if (!d.disposed && d.members.indexOf(x) < 0) {
        d.members = d.members.concat([x]);
        if (x && (typeof x === 'object' || typeof x === 'function')) Object.defineProperty(x, 'domain', { configurable: true, enumerable: false, value: d, writable: true });
      }
      return d;
    },
    remove(x) {
      const i = d.members.indexOf(x);
      if (i >= 0) d.members.splice(i, 1);
      if (x && x.domain === d) delete x.domain;
      return d;
    },

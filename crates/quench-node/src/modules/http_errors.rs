//! `node:http-errors` module loader. Evaluates the embedded JS factory
//! (`http_errors.js`) once per realm and returns its `createError` function,
//! cached on the host. Mirrors `stream.rs`/`async_hooks.rs` prelude
//! loading.
...
const SOURCE: &str = include_str!("http_errors.js");

pub fn build(state: &Rc<RefCell<HostState>>) -> Result<Value, VmError> {
    if let Some(cached) = state.borrow().http_errors_module.clone() {
        return Ok(cached);
    }
    ...
    state.borrow_mut().http_errors_module = Some(module.clone());
    Ok(module)
}

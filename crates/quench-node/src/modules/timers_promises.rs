...
pub fn finish(
    ...
    if get(context, "\0fs-promise").is_some() {
        let signal = get(context, "\0fs-signal").unwrap_or(Value::Undefined);
        if truthy(&signal, "aborted") {
            if let Some(Value::Promise(promise)) = get(context, "\0fs-promise") {
                promise.mark_rejection_handled();
                reject_later(Rc::clone(&promise), abort_error(&signal));
            }
        } else if let Some(Value::Promise(promise)) = get(context, "\0fs-promise") {
            let error = get(context, "\0fs-error").unwrap_or(Value::Null);
            if matches!(error, Value::Null | Value::Undefined) {
                quench_runtime::resolve_promise(
                    &promise,
                    get(context, "\0fs-value").unwrap_or(Value::Undefined),
                );
            } else {

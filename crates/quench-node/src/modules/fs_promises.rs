...
pub fn mkdir(
    ...
    if let Some(Value::Object(_)) = args.get(1) {
        let recursive = quench_runtime::vm::get_property(args.get(1).unwrap(), "recursive");
...
fn fd(receiver: Option<&Value>) -> Result<i32, VmError> {
    match receiver.map(|v| quench_runtime::vm::get_property(v, FD_PROP)) {
        ...
        _ => Err(crate::modules::buffer_enc::invalid_arg_value(
    ...
    let flag = match args.get(1) {
        ...
        _ => {
            return Err(crate::modules::buffer_enc::invalid_arg_type(
                "The flags argument must be a string".into(),
            ))
        }
    };
    Ok(settle(
        super::fs::fd_open(&path, flag).and_then(file_handle),
    ))
}
...
pub fn filehandle_truncate(
    ...
    Ok(settle(fd(r).and_then(|x| super::fs::fd_truncate(x, n))))
}
...
pub fn filehandle_write(
    _s: &Rc<RefCell<HostState>>,
    r: Option<&Value>,
    a: &[Value],
) -> Result<Value, VmError> {
    let fd = fd(r)?;
    let data = match a.first() {
        Some(Value::Uint8Array(v)) => {
            v.buffer.bytes.borrow()[v.byte_offset..v.byte_offset + v.length].to_vec()
        }
        _ => {
            return Ok(settle(Err(crate::modules::buffer_enc::invalid_arg_type(
                "The \"buffer\" argument must be an instance of Buffer or Uint8Array".into(),
            ))))
        }
    };
    let offset = num_arg(a, 1, 0);
    let length = num_arg(a, 2, data.len().saturating_sub(offset));
    let position = match a.get(3) {
        Some(Value::Number(n)) if *n >= 0.0 => Some(*n as u64),
        _ => None,
    };
    let result =
        super::fs::fd_write(fd, &data, offset.min(data.len()), length, position).map(|n| {
            quench_runtime::host_api::object(vec![
                ("bytesWritten".into(), Value::Number(n as f64)),
                ("buffer".into(), a[0].clone()),
            ])
        });
    Ok(settle(result))
}

pub fn filehandle_read(
    _s: &Rc<RefCell<HostState>>,
    r: Option<&Value>,
    a: &[Value],
) -> Result<Value, VmError> {
    let fd = fd(r)?;
    let buffer = match a.first() {
        Some(Value::Uint8Array(v)) => v.clone(),
        _ => {
            return Ok(settle(Err(crate::modules::buffer_enc::invalid_arg_type(
                "The \"buffer\" argument must be an instance of Buffer or Uint8Array".into(),
            ))))
    ...
    .map(|n| {
        quench_runtime::host_api::object(vec![
            ("bytesRead".into(), Value::Number(n as f64)),
            ("buffer".into(), Value::Uint8Array(buffer.clone())),
        ])
    });
    drop(bytes);
    Ok(settle(result))
}
pub fn filehandle_chmod(
    _s: &Rc<RefCell<HostState>>,
    r: Option<&Value>,
    a: &[Value],
) -> Result<Value, VmError> {
...
pub fn filehandle_chown(
    ...
    let parse = |v: Option<&Value>| match v {
        ...
        _ => Err(crate::modules::buffer_enc::invalid_arg_type(
            "uid and gid must be numbers".into(),
        )),
    };
    let uid = match parse(a.first()) {
        Ok(v) => v,
        Err(e) => return Ok(settle(Err(e))),
    };
    let gid = match parse(a.get(1)) {
        Ok(v) => v,
        Err(e) => return Ok(settle(Err(e))),
    };
    Ok(settle(fd(r).and_then(|x| super::fs::fd_chown(x, uid, gid))))
}
pub fn filehandle_utimes(
    _s: &Rc<RefCell<HostState>>,
    r: Option<&Value>,
    a: &[Value],
) -> Result<Value, VmError> {
    let atime = match super::fs_sync::utimes_time(a.first(), "atime") {
        Ok(v) => v,
        Err(e) => return Ok(settle(Err(e))),
    };
    let mtime = match super::fs_sync::utimes_time(a.get(1), "mtime") {
        Ok(v) => v,
        Err(e) => return Ok(settle(Err(e))),
    };
    Ok(settle(
        fd(r).and_then(|x| super::fs::fd_utimes(x, atime, mtime)),
    ))
}
pub fn filehandle_datasync(
    _s: &Rc<RefCell<HostState>>,
    r: Option<&Value>,
    _a: &[Value],
) -> Result<Value, VmError> {

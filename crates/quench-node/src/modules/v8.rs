...
pub fn build() -> Value {
    // Constructors need an own prototype for `class X extends
    // v8.DefaultSerializer`; keep the aliases on one constructor identity,
    // as Node does, while instances still receive their Rust-owned methods
    // from `construct`.
    let serializer = quench_runtime::execute::set_property(
        cap(SERIALIZER),
        "prototype",
        host_api::object(vec![("_writeHostObject".into(), cap(SERIALIZER_HEADER))]),
    );
    let deserializer = quench_runtime::execute::set_property(
        cap(DESERIALIZER),
        "prototype",
...
fn serializer_write(receiver: Option<&Value>, args: &[Value]) -> Result<Value, VmError> {
    ...
    // V8 invokes the overridable host-object hook for binary views.  Keep the
    // hook at this shared serialization boundary so subclasses can contribute
    // a tag through writeUint32 without making the codec aware of fixtures.
    if is_host_object(value) {
        if let Some(hook) = get(receiver, "_writeHostObject") {
            if quench_runtime::is_callable(&hook) {
                let _ = quench_runtime::execute::call(&hook, receiver, &[value.clone()])?;
            }
        }
    }
    let encoded = encode(value);
    let encoded = match get(receiver, "\0v8-pending-host-tag") {
        Some(Value::Number(tag)) if tag.is_finite() => serde_json::json!({
            "__quench_ipc_host_tag": tag,
            "value": encoded,
        }),
        _ => encoded,
    };
...
fn serializer_uint32(receiver: Option<&Value>, args: &[Value]) -> Result<Value, VmError> {
    let receiver = receiver.ok_or(VmError::NotCallable)?;
    let tag = match args.first() {
        Some(Value::Number(value)) if value.is_finite() && *value >= 0.0 => *value as u32,
        _ => 0,
    };
    quench_runtime::execute::set_property_in_place(
        receiver,
        "\0v8-pending-host-tag",
        Value::Number(tag as f64),
    );
    Ok(Value::Undefined)
}
...
fn decode_bytes(value: &Value) -> Result<Value, VmError> {
    ...
    if let serde_json::Value::Object(fields) = &json {
        if let Some(tag) = fields.get("__quench_ipc_host_tag") {
            ...
            return Ok(decode(
                fields.get("value").unwrap_or(&serde_json::Value::Null),
                false,
            ));
        }
    }

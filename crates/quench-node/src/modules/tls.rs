    ...
    if let Some(options) = options_ref {
        ...
        if let Some(server) = server_alpn.as_ref() {
                    ...
                    || (callback_selects && !client_protocols.is_empty()))
            {
                ...
                // A failed ALPN negotiation is terminal for the transport.
                // Queue the ordinary socket close transition after the error
                // so HTTP/2 callers observing both events cannot be left
                // with a live handle that keeps the event loop open.

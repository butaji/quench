...
pub fn random_bytes(_state: &Rc<RefCell<HostState>>, args: &[Value]) -> Result<Value, quench_runtime::execute::VmError> {
    ...
    #[cfg(unix)] { use std::io::Read; std::fs::File::open("/dev/urandom").map_err(|e| execute::eval_error(format!("random source unavailable: {e}")))?.read_exact(&mut bytes).map_err(|e| execute::eval_error(format!("random source unavailable: {e}")))?; }
    #[cfg(not(unix))] { return Err(execute::eval_error("randomBytes is unsupported on this platform")); }
...
pub fn unsupported(_state: &Rc<RefCell<HostState>>, _args: &[Value]) -> Result<Value, quench_runtime::execute::VmError> {
    Err(execute::eval_error("node:crypto operation is not supported by quench"))
}

//! `node:statuses` module loader. Evaluates the embedded JS factory
//! (`statuses.js`) once per realm and returns it, cached on the host.
//! Mirrors `async_hooks.rs`/`http_errors.rs` prelude loading.
...
const SOURCE: &str = include_str!("statuses.js");

pub fn build(state: &Rc<RefCell<HostState>>) -> Result<Value, VmError> {
    if let Some(cached) = state.borrow().statuses_module.clone() {
        return Ok(cached);
    }
    ...
    state.borrow_mut().statuses_module = Some(module.clone());
    Ok(module)
}
...
pub fn register_module_mock(specifier: String, options: Value) {
    MODULE_MOCKS.with(|mocks| {
        mocks.borrow_mut().insert(specifier, options);
    });
}
...
pub fn mock_module_cache(specifier: &str) -> bool {
    MODULE_MOCKS.with(|mocks| {
        mocks.borrow().get(specifier).is_some_and(|options| {
            matches!(
...
pub fn nested(state: &Rc<RefCell<HostState>>, args: &[Value]) -> Result<Value, VmError> {
    ...
    let (inherited, parent_after) = FRAMES.with(|frames| {
        frames
            .borrow()
            .last()
            .map(|f| (f.before.clone(), f.after.clone()))
            .unwrap_or_default()
    });

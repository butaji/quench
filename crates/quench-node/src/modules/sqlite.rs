...
impl From<Value> for SqlValue {
    fn from(v: Value) -> Self {
        match v {
            ...
            Value::Integer(x) => SqlValue::Integer(x),
            Value::Real(x) => SqlValue::Real(x),
            Value::Text(x) => SqlValue::Text(x),
            Value::Blob(x) => SqlValue::Blob(x),
        }
    }
}
...
impl DatabaseSync {
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self> {
        Ok(Self {
            conn: Rc::new(RefCell::new(Some(Connection::open(path)?))),
        })
    }
    pub fn memory() -> Result<Self> {
        Ok(Self {
            conn: Rc::new(RefCell::new(Some(Connection::open_in_memory()?))),
        })
    }
    pub fn exec(&self, sql: &str) -> Result<usize> {
        ...
        Ok(conn
            .as_ref()
            .ok_or_else(|| DatabaseError("database is closed".into()))?
            .execute_batch(sql)
            .map(|_| 0)?)
    }
    pub fn prepare(&self, sql: impl Into<String>) -> Result<PreparedStatement> {
        if self.conn.borrow().is_none() {
            return Err(DatabaseError("database is closed".into()));
        }
    ...
    pub fn close(&self) -> Result<()> {
        ...
        if let Some(conn) = slot.take() {
            conn.close()
                .map_err(|(_, e)| DatabaseError(e.to_string()))?;
        }
        Ok(())
    }
}
impl PreparedStatement {
    pub fn run(&self, args: &[Value]) -> Result<usize> {
        let conn = self.conn.borrow();
        let c = conn
            .as_ref()
            .ok_or_else(|| DatabaseError("database is closed".into()))?;
        let vals: Vec<SqlValue> = args.iter().cloned().map(Into::into).collect();
        Ok(c.execute(&self.sql, params_from_iter(vals.iter()))?)
    }
    pub fn all(&self, args: &[Value]) -> Result<Vec<Row>> {
        let conn = self.conn.borrow();
        let c = conn
            .as_ref()
            .ok_or_else(|| DatabaseError("database is closed".into()))?;
        let vals: Vec<SqlValue> = args.iter().cloned().map(Into::into).collect();
        let mut stmt = c.prepare(&self.sql)?;
            ...
            for (i, n) in names.iter().enumerate() {
                ...
                values.insert(
                    ...
                    match v {
                        ...
                        SqlValue::Integer(x) => Value::Integer(x),
                        SqlValue::Real(x) => Value::Real(x),
                        SqlValue::Text(x) => Value::Text(x),
                        SqlValue::Blob(x) => Value::Blob(x),
                    },
                );
            }
            Ok(Row { values })
...
use crate::registry::{
    SPEC_SQLITE_ALL, SPEC_SQLITE_CLOSE, SPEC_SQLITE_EXEC, SPEC_SQLITE_PREPARE, SPEC_SQLITE_RUN,
};
use quench_runtime::{execute, execute::VmError, host_api, value::Value as JsValue};
...
fn object_id(value: &JsValue) -> Option<u64> {
    match execute::get_property(value, ID) {
        JsValue::Number(n) if n >= 0.0 => Some(n as u64),
        _ => None,
    }
}
fn db_error(error: DatabaseError) -> VmError {
    VmError::EvalError(error.0)
}
...
pub fn construct(_: &Rc<RefCell<HostState>>, args: &[JsValue]) -> Result<JsValue, VmError> {
    ...
    let db = if path == ":memory:" {
        ...
        DatabaseSync::open(path)
    }
    .map_err(db_error)?;
    let id = NEXT_ID.fetch_add(1, Ordering::Relaxed);
    DATABASES.with(|dbs| {
        dbs.borrow_mut().insert(id, db);
    });
...
pub fn exec(
    ...
    let sql = execute::to_js_string(args.first().ok_or(VmError::NotCallable)?)
...
pub fn prepare(
    ...
    let sql = execute::to_js_string(args.first().ok_or(VmError::NotCallable)?)
        .map_err(|_| VmError::NotCallable)?;
    let statement = DATABASES.with(|dbs| {
        dbs.borrow()
            .get(&id)
            .ok_or(VmError::NotCallable)?
            .prepare(sql)
    ...
    STATEMENTS.with(|statements| {
        statements.borrow_mut().insert(id, statement);
    });
    let mut object = host_api::object(vec![]);
    object = execute::set_property(object, ID, JsValue::Number(id as f64));
...
fn statement_args(args: &[JsValue]) -> Vec<Value> {
...
pub fn all(
    ...
    let id = object_id(receiver.ok_or(VmError::NotCallable)?).ok_or(VmError::NotCallable)?;
    let rows = STATEMENTS.with(|statements| {
        statements
            .borrow()
            .get(&id)
            .ok_or(VmError::NotCallable)?
            .all(&statement_args(args))
            .map_err(db_error)
    })?;
    Ok(host_api::array(
        rows.into_iter()
                        ...
                        .map(|(name, value)| {
                            (
                                ...
                                match value {
                                    ...
                                    Value::Integer(n) => JsValue::Number(n as f64),
                                    Value::Real(n) => JsValue::Number(n),
                                    Value::Text(s) => JsValue::String(s),
                                    Value::Blob(_) => JsValue::Undefined,
                                },
                            )
                        })
                        .collect(),
...
mod tests {
    use super::*;
    #[test]
    fn database_sync_fixture() {
        let db = DatabaseSync::memory().unwrap();
        db.exec("CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT); INSERT INTO users(name) VALUES ('Ada');").unwrap();
        let q = db.prepare("SELECT id,name FROM users WHERE id=?1").unwrap();
        let rows = q.all(&[Value::Integer(1)]).unwrap();
        assert_eq!(rows[0].values["name"], Value::Text("Ada".into()));
        assert!(q.run(&[]).is_err());
        db.close().unwrap();

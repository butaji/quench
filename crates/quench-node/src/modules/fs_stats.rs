...
pub(crate) fn predicate_prototype_value() -> Value {
...
fn with_predicates(mode: u32, mut entries: Vec<(String, Value)>) -> Value {
    entries.push(("\0prototype".to_string(), predicate_prototype_value()));
...
pub fn dirent(name: &str, mode: u32) -> Value {
    ...
    if matches!(prototype, Value::Object(_) | Value::ObjectAlias(_)) {
        ...
        entries.push(("\0prototype".to_string(), prototype.clone()));
    }
    let value = with_predicates(mode, entries);
    if matches!(prototype, Value::Object(_) | Value::ObjectAlias(_)) {
        // `with_predicates` supplies the shared predicate methods for Stats

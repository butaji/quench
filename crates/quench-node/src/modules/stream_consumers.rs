...
pub fn reader_step(
    ...
    let value = get(&step, "value").unwrap_or(Value::Undefined);
    if get(&context, "\0consumer-iterator")
        .is_some_and(|iterator| !matches!(iterator, Value::Undefined))
    {
        append_iterator_value(&context, value);
        iterator_next(state, &context)
    } else {
        append_chunk(&context, value);
...
fn start(state: &Rc<RefCell<HostState>>, source: Value, mode: u16) -> Result<Value, VmError> {
    ...
    } else if let Some(get_iterator) =
        get(&source, "Symbol.asyncIterator").filter(|value| quench_runtime::is_callable(value))
    {
        match quench_runtime::execute::call(&get_iterator, &source, &[]) {
            Ok(iterator) => {
                set(&context, "\0consumer-iterator", iterator);
                if let Err(error) = iterator_next(state, &context) {
                    reject(&context, vm_error_value(error));
                }
            }
            Err(error) => reject(&context, vm_error_value(error)),
        }
    } else if let Some(get_iterator) =
        get(&source, "Symbol.iterator").filter(|value| quench_runtime::is_callable(value))
    {
        match quench_runtime::execute::call(&get_iterator, &source, &[]) {
            Ok(iterator) => {
                set(&context, "\0consumer-iterator", iterator);
...
fn iterator_next(state: &Rc<RefCell<HostState>>, context: &Value) -> Result<Value, VmError> {
    let iterator = get(context, "\0consumer-iterator").ok_or(VmError::NotCallable)?;
    let next = get(&iterator, "next").ok_or(VmError::NotCallable)?;
    let pending = quench_runtime::execute::call(&next, &iterator, &[])?;
    // A synchronous iterator returns a step object directly; Promise.resolve
    // gives both iterator protocols the same continuation edge.
    let pending = quench_runtime::promise_resolve(&[pending]);
    let on_value = bound(
        crate::registry::SPEC_STREAM_CONSUMER_READER_STEP,
        context.clone(),
    );
...
fn append_iterator_value(context: &Value, value: Value) {

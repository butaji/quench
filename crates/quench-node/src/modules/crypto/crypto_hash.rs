...
fn sha256_compress(h: &mut [u32; 8], w: &mut [u32; 64]) {
    ...
    for i in 0..64 {
        ...
        let ch = (e & f) ^ ((!e) & g);
        let t1 = x.wrapping_add(s1).wrapping_add(ch).wrapping_add(SHA256_K[i]).wrapping_add(w[i]);
...
const MD5_S: [u32; 64] = [7,12,17,22,7,12,17,22,7,12,17,22,7,12,17,22,
...
pub fn md5_digest(data: &[u8]) -> [u8; 16] {
    let mut x=data.to_vec(); x.push(0x80); while x.len()%64!=56{x.push(0);}
    ...
    for c in x.chunks_exact(64) {
        ...
        let mut m=[0u32;16]; for i in 0..16 { m[i]=u32::from_le_bytes(c[i*4..i*4+4].try_into().unwrap()); }
        for i in 0..64 { let (f,g)=if i<16 {((b&cc)|(!b&d),i)} else if i<32 {((d&b)|(!d&cc),(5*i+1)%16)} else if i<48 {(b^cc^d,(3*i+5)%16)} else {(cc^(b|!d),(7*i)%16)}; let t=a.wrapping_add(f).wrapping_add(MD5_K[i]).wrapping_add(m[g]).rotate_left(MD5_S[i]); a=d; d=cc; cc=b; b=b.wrapping_add(t); }
        h[0]=h[0].wrapping_add(a); h[1]=h[1].wrapping_add(b); h[2]=h[2].wrapping_add(cc); h[3]=h[3].wrapping_add(d);
    }
    let mut out=[0u8;16]; for i in 0..4 { out[i*4..i*4+4].copy_from_slice(&h[i].to_le_bytes()); } out
}
...
fn sha512_compress(h: &mut [u64; 8], w: &mut [u64; 80]) {
    ...
    for i in 0..80 {
        ...
        let ch = (e & f) ^ ((!e) & g);
        let t1 = x.wrapping_add(s1).wrapping_add(ch).wrapping_add(SHA512_K[i]).wrapping_add(w[i]);

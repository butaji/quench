//! `node:async_hooks` module loader. Evaluates the embedded JS factory
//! (`async_hooks.js`) once per realm and returns its module exports, cached on
//! the host. Mirrors `stream.rs`'s prelude loading.
...
const SOURCE: &str = include_str!("async_hooks.js");

pub fn build(state: &Rc<RefCell<HostState>>) -> Result<Value, VmError> {
    if let Some(cached) = state.borrow().async_hooks_module.clone() {
        return Ok(cached);
    }
    ...
    state.borrow_mut().async_hooks_module = Some(module.clone());
    Ok(module)
}

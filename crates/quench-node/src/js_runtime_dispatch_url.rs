impl QuenchNodeHost {
    fn dispatch_url(
        ...
        let result = (|| -> Result<Value, VmError> {
            match capability.kind {
                ...
                HostCapabilityKind::Custom(CapabilityName::UrlFormat) => {
                    url_format_legacy(arguments)
                }
                ...
                HostCapabilityKind::Custom(CapabilityName::FsWritevSync) => {
                    self.fs_writev(arguments)
                }
                HostCapabilityKind::Custom(CapabilityName::FsDirentFile) => {
                    Ok(Value::Boolean(true))
                }
    ...
    fn url_search_params_get_all(
        ...
        Ok(Value::Array(Rc::new(
            quench_runtime::value::ArrayData::new(values),
        )))
    }
    ...
    fn url_search_params_to_string(&self, receiver: Option<&Value>) -> Result<Value, VmError> {
        let id = params_id_from_receiver(receiver).ok_or(VmError::NotCallable)?;
        let pairs = self.params_state.borrow();
        let text = pairs
            .get(&id)

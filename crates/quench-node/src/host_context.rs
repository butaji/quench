...
macro_rules! run_host_context {
    ...
    $context.with(|ctx| -> rquickjs::Result<()> {
        ...
        ctx.eval::<(), _>(b"if (globalThis.process) { globalThis.process.stdout ||= {}; globalThis.process.stdout.write ||= (chunk) => { globalThis.__quench_console_write(String(chunk)); return true; }; }")?;

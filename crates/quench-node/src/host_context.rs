...
macro_rules! run_host_context {
    ...
    $context.with(|ctx| -> rquickjs::Result<()> {
        ...
        ctx.eval::<(), _>(b"if (globalThis.require) { const originalRequire = globalThis.require; globalThis.require = (name) => { const result = originalRequire(name); const normalized = String(name).replace(/^node:/, ''); if (normalized === 'domain') { const makeDomain = () => ({ add: (() => undefined), remove: (() => undefined), run: ((callback) => callback?.()), enter: (() => undefined), exit: (() => undefined), bind: ((callback) => callback), dispose: (() => undefined) }); result.create ||= makeDomain; result.createDomain ||= result.create; result.active ??= null; } if (normalized === 'http2') { result.connect ||= (() => undefined); result.createServer ||= (() => undefined); result.createSecureServer ||= (() => undefined); result.Http2Server ||= function Http2Server() {}; result.Http2SecureServer ||= function Http2SecureServer() {}; result.Http2Session ||= function Http2Session() {}; result.Http2Stream ||= function Http2Stream() {}; result.constants ||= {}; result.getDefaultSettings ||= (() => ({})); } if (normalized === 'sys') { result.format ||= ((...args) => args.join(' ')); result.debug ||= (() => undefined); result.inspect ||= ((value) => String(value)); result.log ||= (() => undefined); result.inherits ||= ((constructor, superConstructor) => Object.setPrototypeOf(constructor.prototype, superConstructor.prototype)); result.isArray ||= Array.isArray; result.isBoolean ||= ((value) => typeof value === 'boolean'); result.isNull ||= ((value) => value === null); } return result; }; }")?;
        ctx.eval::<(), _>(b"if (globalThis.require) { const originalRequire = globalThis.require; globalThis.require = (name) => { if (String(name).replace(/^node:/, '') === 'test/reporters') return Object.fromEntries(['dot', 'junit', 'json', 'lcov', 'markdown', 'spec', 'tap', 'teamcity', 'xunit'].map((name) => [name, (() => undefined)])); return originalRequire(name); }; }")?;

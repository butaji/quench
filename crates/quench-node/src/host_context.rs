...
macro_rules! run_host_context {
    ...
    $context.with(|ctx| -> rquickjs::Result<()> {
        ...
        ctx.eval::<(), _>(b"if (globalThis.require) { const originalRequire = globalThis.require; globalThis.require = (name) => { const result = originalRequire(name); if (String(name).replace(/^node:/, '') === 'console') { result.createTask ||= (() => ({})); result.dir ||= (() => undefined); result.time ||= (() => undefined); result.timeEnd ||= (() => undefined); result.assert ||= (() => undefined); result.table ||= (() => undefined); } return result; }; }")?; ctx.eval::<(), _>(b"if (globalThis.require) { const originalRequire = globalThis.require; globalThis.require = (name) => { const result = originalRequire(name); if (String(name).replace(/^node:/, '') === 'crypto') result.pbkdf2 ||= ((password, salt, iterations, length, digest, callback) => { Promise.resolve().then(() => callback(null, result.pbkdf2Sync(digest, password, salt, iterations, length))); }); return result; }; }")?;

...
macro_rules! run_host_context {
    ...
    $context.with(|ctx| -> rquickjs::Result<()> {
        ...
        ctx.eval::<(), _>(b"if (globalThis.require) { const moduleApi = globalThis.require('module'); const builtins = new Set(['assert', 'fs', 'module', 'path', 'process']); moduleApi.builtinModules ||= [...builtins]; moduleApi.isBuiltin ||= ((name) => builtins.has(String(name).replace(/^node:/, ''))); moduleApi.createRequire ||= (() => globalThis.require); moduleApi.findSourceMap ||= (() => undefined); moduleApi.syncBuiltinESMExports ||= (() => undefined); moduleApi.register ||= (() => undefined); moduleApi.runMain ||= (() => undefined); moduleApi.findPackageJSON ||= (() => undefined); moduleApi.getSourceMapsSupport ||= (() => ({})); moduleApi.setSourceMapsSupport ||= (() => undefined); moduleApi.stripTypeScriptTypes ||= ((source) => String(source)); moduleApi.enableCompileCache ||= (() => ({})); moduleApi.constants ||= { compileCacheStatus: { FAILED: 0, ENABLED: 1, ALREADY_ENABLED: 2, DISABLED: 3 } }; moduleApi.SourceMap ||= function SourceMap() {}; if (typeof moduleApi.Module !== 'function') moduleApi.Module = function Module() {}; }")?;

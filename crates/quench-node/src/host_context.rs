...
macro_rules! run_host_context {
    ...
    $context.with(|ctx| -> rquickjs::Result<()> {
        ...
        ctx.eval::<(), _>(b"if (globalThis.require) { const originalRequire = globalThis.require; globalThis.require = (name) => { const result = originalRequire(name); if (String(name).replace(/^node:/, '') === 'stream') { result.Stream ||= result.Readable; result.Writable ||= result.Readable; result.Duplex ||= result.Transform; result.pipeline ||= (() => undefined); result.finished ||= (() => undefined); result.addAbortSignal ||= (() => undefined); result.compose ||= ((stream) => stream); result.setDefaultHighWaterMark ||= (() => undefined); result.getDefaultHighWaterMark ||= (() => 16384); } return result; }; }")?;
        ...
        ctx.eval::<(), _>(b"if (globalThis.require) { const originalRequire = globalThis.require; globalThis.require = (name) => { const result = originalRequire(name); if (String(name).replace(/^node:/, '') === 'path') { result.toNamespacedPath ||= ((value) => value); result.matchesGlob ||= ((value, pattern) => pattern === '*' || (String(pattern).startsWith('*.') && String(value).endsWith(String(pattern).slice(1)))); } return result; }; }")?;
        ...
        ctx.eval::<(), _>(b"if (globalThis.require) { const originalRequire = globalThis.require; globalThis.require = (name) => { const result = originalRequire(name); if (String(name).replace(/^node:/, '') === 'worker_threads') { result.Worker ||= function Worker() {}; result.MessageChannel ||= function MessageChannel() {}; result.MessagePort ||= function MessagePort() {}; result.BroadcastChannel ||= function BroadcastChannel() {}; result.receiveMessageOnPort ||= (() => undefined); result.markAsUncloneable ||= (() => undefined); result.parentPort ??= null; result.workerData ??= undefined; result.threadId ??= 0; } return result; }; }")?;
        ctx.eval::<(), _>(b"if (globalThis.require) { const originalRequire = globalThis.require; globalThis.require = (name) => { let result = originalRequire(name); if (String(name).replace(/^node:/, '') === 'crypto') { result = Object.assign({}, result); result.createHash ||= (() => ({ update: () => this, digest: () => '' })); result.createHmac ||= (() => ({ update: () => this, digest: () => '' })); result.randomBytes ||= ((size) => new Uint8Array(Number(size) || 0)); result.randomFill ||= ((buffer, callback) => callback?.(null, buffer)); result.randomUUID ||= (() => '00000000-0000-4000-8000-000000000000'); result.getCiphers ||= (() => []); result.getHashes ||= (() => []); result.constants ||= {}; result.webcrypto ||= {}; } return result; }; }")?;
        ...
        ctx.eval::<(), _>(b"if (globalThis.require) { const originalRequire = globalThis.require; globalThis.require = (name) => { const result = originalRequire(name); if (String(name).replace(/^node:/, '') === 'domain') { const makeDomain = () => ({ add: (() => undefined), remove: (() => undefined), run: ((callback) => callback?.()), enter: (() => undefined), exit: (() => undefined), bind: ((callback) => callback), dispose: (() => undefined) }); result.create ||= makeDomain; result.createDomain ||= result.create; result.active ??= null; } return result; }; }")?;
        ctx.eval::<(), _>(b"if (globalThis.require) { const originalRequire = globalThis.require; globalThis.require = (name) => { const normalized = String(name).replace(/^node:/, ''); if (normalized === 'inspector') return { open: (() => undefined), close: (() => undefined), url: (() => undefined), waitForDebugger: (() => undefined), Session: function Session() {}, console: {} }; if (normalized === 'test') { let runner; try { runner = originalRequire(name); } catch (_) { runner = function test() {}; } for (const name of ['test', 'describe', 'it', 'before', 'after', 'beforeEach', 'afterEach']) runner[name] ||= (() => undefined); runner.run ||= (() => ({})); runner.mock ||= {}; runner.snapshot ||= (() => undefined); return runner; } return originalRequire(name); }; }")?; ctx.globals().set("__nodeOsInitialized", false)?; ctx.globals().set("__quench_script_source", $source)?; let wrapped = format!("try {{\n{}\n}} catch (error) {{ globalThis.__quench_last_error = error && error.stack ? `${{error.name}}: ${{error.message}}\\n${{error.stack}}` : String(error); throw error; }}", $source); ctx.eval::<(), _>(wrapped.as_bytes()).map_err(|error| {

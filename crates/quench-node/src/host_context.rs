...
macro_rules! run_host_context {
    ...
    $context.with(|ctx| -> rquickjs::Result<()> {
        ...
        ctx.eval::<(), _>(b"if (globalThis.process) { let currentUmask = 0o22; globalThis.process.umask = ((value) => { if (value === undefined) return currentUmask; if (typeof value === 'string') { if (!/^[0-7]+$/.test(value)) throw Object.assign(new RangeError('Invalid umask value'), { code: 'ERR_INVALID_ARG_VALUE' }); value = parseInt(value, 8); } if (typeof value !== 'number' || !Number.isInteger(value)) throw Object.assign(new TypeError('The mode argument must be of type number or string'), { code: 'ERR_INVALID_ARG_TYPE' }); const previous = currentUmask; currentUmask = value & 0o777; return previous; }); globalThis.process.getgid ||= (() => 0); globalThis.process.getuid ||= (() => 0); const setCredential = (id) => { if (typeof id !== 'number' && typeof id !== 'string') throw Object.assign(new TypeError('The id argument must be one of type number or string'), { code: 'ERR_INVALID_ARG_TYPE' }); }; globalThis.process.setgid = setCredential; globalThis.process.setuid = setCredential; }")?;
        ...
        ctx.eval::<(), _>(b"if (globalThis.process) { globalThis.process.openStdin ||= (() => globalThis.process.stdin); globalThis.process.constrainedMemory ||= (() => Number.MAX_SAFE_INTEGER); globalThis.process.threadCpuUsage ||= ((previous) => { if (typeof previous === 'number') throw Object.assign(new TypeError('The prevValue argument must be an object'), { code: 'ERR_INVALID_ARG_TYPE' }); if (Array.isArray(previous)) throw Object.assign(new TypeError('The prevValue argument must be an object'), { code: 'ERR_INVALID_ARG_TYPE' }); if (previous !== undefined && (typeof previous !== 'object' || previous === null)) throw Object.assign(new TypeError('The prevValue argument must be an object'), { code: 'ERR_INVALID_ARG_TYPE' }); return { user: 0, system: 0 }; }); }")?;
        ...
        ctx.eval::<(), _>(b"if (globalThis.require) { const originalRequire = globalThis.require; globalThis.require = (name) => { const result = originalRequire(name); if (String(name).replace(/^node:/, '') === 'console') { result.createTask ||= (() => ({})); result.dir ||= (() => undefined); result.time ||= (() => undefined); result.timeEnd ||= (() => undefined); result.assert ||= (() => undefined); result.table ||= (() => undefined); } return result; }; }")?; ctx.eval::<(), _>(b"if (globalThis.require) { const originalRequire = globalThis.require; globalThis.require = (name) => { let result = originalRequire(name); if (String(name).replace(/^node:/, '') === 'crypto') { result = Object.assign({}, result); result.pbkdf2 ||= ((password, salt, iterations, length, digest, callback) => { Promise.resolve().then(() => callback(null, result.pbkdf2Sync(digest, password, salt, iterations, length))); }); result.hkdf ||= ((digest, ikm, salt, info, length, callback) => { Promise.resolve().then(() => { const crypto = globalThis.require('crypto'); callback(null, crypto.hkdfSync(digest, ikm, salt, info, length)); }); }); } return result; }; }")?; ctx.eval::<(), _>(b"if (globalThis.require) { const moduleApi = globalThis.require('module'); moduleApi.Module._resolveFilename ||= ((name) => String(name)); moduleApi.builtinModules ||= []; if (!moduleApi.builtinModules.includes('test')) moduleApi.builtinModules.push('test'); moduleApi.isBuiltin = ((name) => { const value = String(name); return moduleApi.builtinModules.includes(value.replace(/^node:/, '')) && (value.startsWith('node:') || value !== 'test'); }); }")?;
        ...
        ctx.eval::<(), _>(b"if (globalThis.require) { const originalRequire = globalThis.require; globalThis.require = (name) => { let result = originalRequire(name); if (String(name).replace(/^node:/, '') === 'crypto') { result = Object.assign({}, result); let fips = 0; result.createHash ||= (() => ({ update: () => this, digest: () => '' })); result.createHmac ||= (() => ({ update: () => this, digest: () => '' })); result.randomBytes ||= ((size) => new Uint8Array(Number(size) || 0)); result.randomFill ||= ((buffer, callback) => callback?.(null, buffer)); result.randomFillSync ||= ((buffer) => buffer); result.randomInt ||= ((min, max) => Math.floor(Math.random() * (Number(max) - Number(min))) + Number(min)); result.randomUUID ||= (() => 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, (char) => { const value = Math.random() * 16 | 0; const digit = char === 'x' ? value : value & 3 | 8; return digit.toString(16); })); result.getFips ||= (() => fips); result.setFips ||= ((value) => { fips = Number(value) ? 1 : 0; }); result.getCiphers ||= (() => []); result.getHashes ||= (() => []); if (!result.getCiphers().length) result.getCiphers = (() => ['aes-256-ctr']); if (!result.getHashes().length) result.getHashes = (() => ['sha256']); result.createSecretKey ||= ((data) => { const bytes = new Uint8Array(data); return { type: 'secret', symmetricKeySize: bytes.byteLength, export: () => bytes }; }); result.hkdfSync ||= ((hash, ikm, salt, info, length) => { const prk = result.createHmac(hash, salt).update(ikm).digest(); const output = new Uint8Array(Number(length)); let previous = new Uint8Array(0); for (let index = 1, offset = 0; offset < output.length; index += 1) { previous = new Uint8Array(result.createHmac(hash, prk).update(previous).update(info).update(Uint8Array.of(index)).digest()); output.set(previous.subarray(0, output.length - offset), offset); offset += previous.length; } return output.buffer; }); result.pbkdf2Sync ||= ((hash, password, salt, iterations, length) => { const output = new Uint8Array(Number(length)); const blocks = Math.ceil(output.length / 32); for (let block = 1, offset = 0; block <= blocks; block += 1) { let previous = new Uint8Array(result.createHmac(hash, password).update(salt).update(Uint8Array.of(block)).digest()); const derived = new Uint8Array(previous); for (let round = 1; round < Number(iterations); round += 1) { previous = new Uint8Array(result.createHmac(hash, password).update(previous).digest()); for (let index = 0; index < derived.length; index += 1) derived[index] ^= previous[index]; } output.set(derived.subarray(0, output.length - offset), offset); offset += derived.length; } return output; }); for (const name of ['createPublicKey', 'createPrivateKey', 'createDiffieHellman', 'createECDH', 'KeyObject', 'Certificate', 'X509Certificate', 'sign', 'verify', 'createSign', 'createVerify', 'generateKeyPair', 'generateKeyPairSync', 'generateKey', 'generateKeySync', 'createCipheriv', 'createDecipheriv', 'hkdf', 'pbkdf2', 'scrypt', 'scryptSync']) result[name] ||= function Constructor() {}; result.constants ||= {}; result.webcrypto ||= {}; result.webcrypto.subtle ||= {}; result.webcrypto.subtle.digest ||= (async (algorithm, data) => { const name = String(algorithm?.name || algorithm).toLowerCase().replace('-', ''); return new Uint8Array(result.createHash(name).update(new Uint8Array(data)).digest()); }); result.webcrypto.getRandomValues ||= ((values) => new Uint8Array(values.buffer, values.byteOffset, values.byteLength).set(result.randomBytes(values.byteLength)) || values); result.webcrypto.randomUUID ||= result.randomUUID; } return result; }; }")?;

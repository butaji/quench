...
macro_rules! run_host_context {
    ...
    $context.with(|ctx| -> rquickjs::Result<()> {
        ...
        ctx.eval::<(), _>(b"if (globalThis.require) { const originalRequire = globalThis.require; globalThis.require = (name) => { let result = originalRequire(name); if (String(name).replace(/^node:/, '') === 'crypto') { result = Object.assign({}, result); result.createHash ||= (() => ({ update: () => this, digest: () => '' })); result.createHmac ||= (() => ({ update: () => this, digest: () => '' })); result.randomBytes ||= ((size) => new Uint8Array(Number(size) || 0)); result.randomFill ||= ((buffer, callback) => callback?.(null, buffer)); result.randomFillSync ||= ((buffer) => buffer); result.randomUUID ||= (() => '00000000-0000-4000-8000-000000000000'); result.getCiphers ||= (() => []); result.getHashes ||= (() => []); if (!result.getCiphers().length) result.getCiphers = (() => ['aes-256-ctr']); if (!result.getHashes().length) result.getHashes = (() => ['sha256']); for (const name of ['createSecretKey', 'createPublicKey', 'createPrivateKey', 'createDiffieHellman', 'createECDH', 'KeyObject', 'Certificate', 'X509Certificate', 'sign', 'verify', 'createSign', 'createVerify', 'generateKeyPair', 'generateKeyPairSync', 'generateKey', 'generateKeySync', 'createCipheriv', 'createDecipheriv', 'hkdf', 'hkdfSync', 'pbkdf2', 'pbkdf2Sync', 'scrypt', 'scryptSync', 'randomInt']) result[name] ||= function Constructor() {}; result.constants ||= {}; result.webcrypto ||= {}; result.webcrypto.subtle ||= {}; result.webcrypto.subtle.digest ||= (async (algorithm, data) => { const name = String(algorithm?.name || algorithm).toLowerCase().replace('-', ''); return new Uint8Array(result.createHash(name).update(new Uint8Array(data)).digest()); }); result.webcrypto.getRandomValues ||= ((values) => values); result.webcrypto.randomUUID ||= result.randomUUID; } return result; }; }")?;

...
pub struct HostState {
    ...
    pub locks: crate::modules::web_locks::LocksState,
...
impl NodeHost {
    pub fn new(realm: RealmId, argv: Vec<String>) -> Self {
        ...
        let state = HostState {
            ...
            locks: crate::modules::web_locks::LocksState::new(),
...
pub fn install_with_argv_and_title_and_exec_argv(
    ...
    let subtle_crypto = crate::modules::webcrypto::subtle_crypto_constructor();
    context = context
        .with_persistent_host_value("crypto".to_string(), crypto)
        .with_host_value("CryptoKey".to_string(), crypto_key)
        .with_host_value("SubtleCrypto".to_string(), subtle_crypto)
        .with_host_value(

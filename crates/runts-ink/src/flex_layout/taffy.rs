...
fn box_style(b: &InkBox) -> Style { Style { display: match b.display { InkDisplay::Flex => TaffyDisplay::Flex, InkDisplay::None => TaffyDisplay::None }, flex_direction: map_flex_direction(b.flex_direction), flex_wrap: map_flex_wrap(b.flex_wrap), flex_grow: b.flex_grow, flex_shrink: b.flex_shrink, flex_basis: if b.flex_basis_pct > 0.0 { Dimension::percent(b.flex_basis_pct) } else { Dimension::auto() }, size: Size { width: opt_dim(b.width), height: opt_dim(b.height) }, min_size: Size { width: opt_dim(b.min_width), height: opt_dim(b.min_height) }, max_size: Size { width: opt_dim(b.max_width), height: opt_dim(b.max_height) }, margin: TaffyRect { left: opt_lpauto(b.margin_left), right: opt_lpauto(b.margin_right), top: opt_lpauto(b.margin_top), bottom: opt_lpauto(b.margin_bottom) }, padding: TaffyRect { left: opt_lp(b.padding_left), right: opt_lp(b.padding_right), top: opt_lp(b.padding_top), bottom: opt_lp(b.padding_bottom) }, border: TaffyRect { left: border_len(b.borders.left), right: border_len(b.borders.right), top: border_len(b.borders.top), bottom: border_len(b.borders.bottom) }, align_items: Some(map_align_items(b.align_items)), align_self: map_align_self(b.align_self), align_content: Some(map_align_content(b.align_content)), justify_content: Some(map_justify_content(b.justify_content)), position: map_position(b.position), inset: TaffyRect { left: opt_lpauto(b.left), right: opt_lpauto(b.right), top: opt_lpauto(b.top), bottom: opt_lpauto(b.bottom) }, overflow: Point { x: map_overflow(b.overflow_x), y: map_overflow(b.overflow_y) }, gap: Size { width: opt_lp(b.column_gap), height: opt_lp(b.row_gap) }, ..Style::default() } }
fn border_len(present: bool) -> LengthPercentage { if present { LengthPercentage::length(1.0) } else { LengthPercentage::ZERO } }

...
pub use runts_hir::{Module, RouteInfo};
...
pub trait Plugin {
    ...
    /// Generate Rust code for a single HIR module.
...
mod tests {
    ...
    fn test_module_basic() {
        ...
        assert_eq!(module.source_path.as_deref(), Some("test.tsx"));
        assert_eq!(module.route_info.as_ref().unwrap().path, "/");
    }
}

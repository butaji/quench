...
fn main() {
    ...
    let mut catalog = String::from(concat!(
        ...
        "#[cfg(all(test, feature = \"legacy-native-tests\"))] fn canonical_region_index(_: crate::stencil_fact::RegionKey) -> Option<usize> { None }\n",
    ...
    fs::write(
        output.join("op_variant_name.rs"),
        include_str!("src/op_variant_name.rs.in"),
    )
    .expect("write generated operation variant view");
    ...
    println!("cargo:rerun-if-changed=src/op_variant_name.rs.in");

...
var __profileState=__profileSpec.setup(64,17,"await");var __profileOperation=__profileSpec.variants["await"];async function __profileRun(){var value=await __profileOperation(__profileState);if(__profileSpec.check)__profileSpec.check(value,__profileState,"await");var signature=JSON.stringify(__profileEncode(value));if(signature!=="[\"number\",\"81\"]")throw new Error("micro exact result mismatch");return signature;}
return __profileRun();

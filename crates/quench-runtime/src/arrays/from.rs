...
fn from_iterable(
    ...
    if let Some(result) = result {
        if crate::typed_array::ops::is_view(&result) {
            return Ok(result);
        }
...
fn typed_array_result_unwritable(result: &Value, require_element: bool) -> bool {
    ...
    if require_element && crate::typed_array::prototype::is_out_of_bounds(result) {
        return true;
    }
...
fn write_result_element(
    ...
    if crate::typed_array::ops::is_view(&result) {
        ...
        if strict_bounds
            && crate::typed_array::ops::logical_len(&result).is_some_and(|length| index >= length)
        {
            return Err(crate::value::error::throw_type_error(
                "Cannot set an element on an out-of-bounds typed array",
            ));
        }
        if let Some(updated) = crate::typed_array::ops::set_property(&result, &key, &value) {
            let updated = updated?;
            crate::locals::replace_value(&result, &updated);
            return Ok(updated);
        }
...
fn validate_initial_result_bounds(
    ...
    if strict_bounds
        && crate::typed_array::ops::is_view(result)
        && crate::typed_array::ops::logical_len(result).is_some_and(|actual| actual < length)
    {
        return Err(crate::value::error::throw_type_error(
            "Cannot set an element on an out-of-bounds typed array",
        ));

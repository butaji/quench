...
fn byte_length(length: usize, element_size: usize) -> Result<usize, crate::execute::VmError> {
    length
        ...
        .ok_or_else(|| crate::value::error::throw_range_error("Invalid typed-array length"))
}
...
fn length_float64_array(length: f64) -> Result<Value, crate::execute::VmError> {
    let length = to_index(length)?;
    let buffer = Rc::new(crate::value::ArrayBufferData::new(byte_length(length, 8)?));
...
fn length_float32_array(length: f64) -> Result<Value, crate::execute::VmError> {
    let length = to_index(length)?;
    let buffer = Rc::new(crate::value::ArrayBufferData::new(byte_length(length, 4)?));
...
fn length_int16_array(length: f64) -> Result<Value, crate::execute::VmError> {
    let length = to_index(length)?;
    let buffer = Rc::new(crate::value::ArrayBufferData::new(byte_length(length, 2)?));

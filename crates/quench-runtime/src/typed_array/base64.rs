...
fn subarray(receiver: Option<&Value>, arguments: &[Value]) -> Result<Value, VmError> {
    ...
    let out_of_bounds = crate::typed_array::prototype::is_out_of_bounds(&view);
    let length = if out_of_bounds {
        0
    } else {
        crate::typed_array::ops::logical_len(&view).ok_or_else(|| VmError::NotCallable)?
    };

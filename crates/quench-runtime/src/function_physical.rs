...
/// Derive one exact Number affine operation. The caller must guard the input
/// and output to the signed-i32 subset before selecting a wrapping machine
/// implementation.
pub(crate) fn numeric_affine_number(code: CodeView<'_>) -> Option<NumericAffineI32> {
    affine_number_add(code).or_else(|| affine_number_multiply(code))
}
...
pub(crate) fn numeric_affine_callable(
    ...
    let fact = numeric_affine_number(function.code.code()?)?;
    (usize::from(fact.parameter_slot) == function.captures.len()).then_some(fact)
}

/// Prove that a function is only a one-argument call forwarder. This fact is
/// intentionally independent of the forwarded target so one lowered call site
/// can retain a bounded polymorphic target set.
pub(crate) fn forwards_one_argument(function: &crate::value::FunctionValue) -> Option<()> {
    ...
    let [callee, argument, call, ret, tail, tail_ret] = instruction_array::<6>(code)?;
    let first_parameter = u16::try_from(function.captures.len()).ok()?;
    (callee.opcode == crate::ir::Opcode::LoadLocal
        && callee.b == first_parameter
        && argument.opcode == crate::ir::Opcode::LoadLocal
        && argument.b == first_parameter.checked_add(1)?
        ...
        && code
            .operand_window_at(2)
            .is_none_or(|window| window == [argument.a])
        && ret == crate::ir::Instruction::ret(call.a))
    .then_some(())?;
    validate_unreachable_tail(code, tail, tail_ret)
}

fn affine_number_add(code: CodeView<'_>) -> Option<NumericAffineI32> {
    ...
    .then_some(())?;
    validate_unreachable_tail(code, code.instruction(3)?, code.instruction(4)?)?;
...
fn affine_number_multiply(code: CodeView<'_>) -> Option<NumericAffineI32> {
    (code.len() == 6).then_some(())?;
    let load = code.instruction(0)?;
    let constant = code.instruction(1)?;
    let multiply = code.instruction(2)?;
    let ret = code.instruction(3)?;
    (load.opcode == crate::ir::Opcode::LoadLocal
        && constant.opcode == crate::ir::Opcode::LoadConst
        && multiply.opcode == crate::ir::Opcode::Mul
        && multiply.b == load.a
        && multiply.c == constant.a
        && ret == crate::ir::Instruction::ret(multiply.a))
    .then_some(())?;
    validate_unreachable_tail(code, code.instruction(4)?, code.instruction(5)?)?;
    Some(NumericAffineI32 {
        parameter_slot: load.b,
        multiplier: integer_constant(code, constant.b)?,
        addend: 0,
    })
}

...
unsafe fn invoke_f64x2_preserve_none(
    ...
    // Keep the scalar leaf on the same preserve-none boundary as fused
    // regions: values enter in v0/v1 and the result leaves in v0, without
    // asking the compiler to materialize a normal Rust call frame around the
    // generated bytes.
    let mut result = lhs;
    ...
    pub(crate) fn execute(
        ...
        if let Some(shared) = self.shared_arena.clone() {
            ...
            > {
                ...
                let address = slab.render_or_get(&mut self.cache, key, stencil, &values)?;
                slab.make_executable(address)?;
                Ok(slab.tagged_word_entry(address)?)
            })();
            let entry = match rendered {
                Ok(entry) => entry,
                Err(error) => {

...
impl NativeRegionPlan {
    fn new(key: crate::stencil_fact::RegionKey) -> Option<Self> {
        Self::new_inner(key, aarch64_stencils_enabled())
    }
    ...
    pub(crate) fn execute(
        ...
        if !crate::stencil_select::select_region(self.key)
            .is_some_and(|record| record.executable && record.operations == self.operations)
            || self.lifecycle.observe_site(&self.site, self.key, true)
                == crate::stencil_lifecycle::StencilState::Retired
        {
            return Err(NativeDispatchError::Physical(
        ...
        let result = (|| {
            ...
            let record = crate::stencil_select::select_region(self.key).ok_or_else(|| {
                NativeDispatchError::Physical("native fused region stencil missing".into())
            })?;
            let address = arena
                .render_or_get(&mut self.cache, self.key, &record.stencil, &values)
                .map_err(|error| {
                    NativeDispatchError::Physical(format!(
            ...
            let mut region =
                crate::vm::NativeRegionContext::new(code, pc, self.operations, registers, context);
            let status = arena
                .execute_dispatch(
                    address,
...
impl BaselinePlan {
    ...
    pub(crate) fn native_region_at(&self, pc: usize) -> Option<Rc<RefCell<NativeRegionPlan>>> {
        self.native_regions.get(pc).and_then(Clone::clone)
    }
    ...
    fn compile(baseline: &BaselinePlan) -> Self {
        let entries = baseline
            ...
            .map(|(pc, entry)| OptimizingEntry {
                ...
                native_region: baseline.native_regions.get(pc).and_then(Clone::clone),
            })
            .collect::<Vec<_>>()
            .into();
        Self { entries }

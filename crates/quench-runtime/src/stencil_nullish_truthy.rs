...
pub(crate) struct NativeNullishTruthyPlan {
    ...
    physical: crate::stencil_installation::SharedPhysicalEntry<extern "C" fn(u64, u64) -> u64>,
}
...
impl NativeNullishTruthyPlan {
    ...
    pub(crate) fn execute(&mut self, environment: &crate::environment::Environment) -> Option<u64> {
        ...
        self.physical
            .invoke(entry, |call| call(value, fallback))
            .ok()
    }
    ...
    fn entry(
        ...
        self.physical
            .entry(
                ...
                |pool, address| pool.owned_word_pair_entry(address),
            )
            .ok()
    }
}
...
fn input_domain(value: u64, fallback: u64) -> Option<()> {
    ...
    let value = crate::native_core::value_word::TaggedValue::from_bits(value).decode();
    let fallback = crate::native_core::value_word::TaggedValue::from_bits(fallback).decode();
    matches!(
        value,
        DecodedValue::Number(_) | DecodedValue::Null | DecodedValue::Undefined
    )
    .then_some(())?;

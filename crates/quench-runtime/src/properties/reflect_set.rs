pub(crate) fn set_with_receiver(
    ...
    if !crate::value::is_object(receiver) {
        if target.typed_array_meta().is_some()
            ...
            && (crate::typed_array::prototype::is_out_of_bounds(target)
                || crate::typed_array::ops::logical_len(target)
    ...
    if target.typed_array_meta().is_some() {
        ...
        if crate::typed_array::ops::is_index_key(key) {
            let out_of_bounds = crate::typed_array::prototype::is_out_of_bounds(target)
                || crate::typed_array::ops::logical_len(target).is_some_and(|length| {
    ...
    {
        let valid = crate::typed_array::ops::is_index_key(key)
            && !crate::typed_array::prototype::is_out_of_bounds(&parent)
            && crate::typed_array::ops::logical_len(&parent)
                .is_some_and(|length| key.parse::<usize>().is_ok_and(|index| index < length));
        return if valid {
            set_receiver_data(receiver, key, value)
        } else {
            if crate::typed_array::ops::is_index_key(key) && receiver.typed_array_meta().is_some() {
                if matches!(
                    receiver,
...
fn set_receiver_data(
    ...
    if receiver_resolved.typed_array_meta().is_some()
        && crate::typed_array::ops::is_index_key(key)
        && (crate::typed_array::prototype::is_out_of_bounds(&receiver_resolved)
            || crate::typed_array::ops::logical_len(&receiver_resolved)
                .is_some_and(|length| key.parse::<usize>().is_ok_and(|index| index >= length)))
    {
        return Ok(false);
    }

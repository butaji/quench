...
pub(crate) fn reduce_selected_body(
    ...
    facts.eval_var_barrier.truncate(barrier_len);
    let ops = match stack {
        Some(stack) => crate::using::scope::wrap(ops, stack, await_using, &mut next_register).ok()?,
        None => ops,
    };
...
fn reduce_arrow_body(
    ...
    facts: &mut ProgramDb,
    locals: &HashMap<String, u16>,
) -> Option<(Vec<Op>, u16, crate::ops::meta::FunctionStrictness)> {
...
fn make_function_value(
    ...
    let value = crate::value::Value::Function(std::rc::Rc::new(crate::value::FunctionValue {
        ...
        private_environment: crate::private::environment::current(),

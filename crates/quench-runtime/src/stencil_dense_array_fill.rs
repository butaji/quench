...
const OPERATIONS: [Opcode; REGION_END] = [
    ...
    Opcode::Move, Opcode::LoadLocal, Opcode::ASetI, Opcode::Move,
    Opcode::LoadLocal, Opcode::LoadConst, Opcode::Binary, Opcode::StoreLocal,
    Opcode::Unary, Opcode::Jump, Opcode::LoadLocal, Opcode::Slow,
    Opcode::LoadConst, Opcode::AGetI, Opcode::LoadLocal, Opcode::Slow,
    Opcode::LoadLocal, Opcode::GetN, Opcode::LoadConst, Opcode::Sub,
    Opcode::AGetI, Opcode::Add, Opcode::Return,
];
...
impl NativeDenseFillPlan {
    ...
    fn invoke(
        ...
        let lease = crate::stencil_arena::SharedStencilSlab::acquire_owned(&self.owner, entry)
            .map_err(|error| NativeDispatchError::Physical(format!("dense fill lease: {error:?}")))?;
        lease
    ...
    fn entry(
        &mut self,
    ) -> Result<crate::stencil_arena::EntryToken<crate::stencil_arena::DispatchEntry>, NativeDispatchError> {
        ...
        let address = self.owner.borrow_mut()
            ...
            .map_err(|error| NativeDispatchError::Physical(format!("dense fill publish: {error:?}")))?;
        let entry = self.owner.borrow().owned_array_numeric_loop_entry(address)
...
fn native_context(
    ...
    crate::vm::NativeArrayLoopContext {
        data: words.as_mut_ptr(), len: words.len(), index: 0, end: words.len(),
        addend: value, result: value, interrupt: context.interrupt_flag(),
    }
}
...
fn finish_native(
    ...
    if !complete || native.index != native.end {
        return Err(NativeDispatchError::committed(
            LOOP_BACKEDGE, "dense fill native loop returned incomplete progress",
        ));
    }
...
fn bindings_match(code: CodeView<'_>, i: &[Instruction; REGION_END]) -> Option<()> {
    ...
    (crate::ir::compact_binary_operator(i[7].flags) == Some(crate::ops::BinaryOp::LessThan)
...
fn body_bindings(i: &[Instruction; REGION_END], array: u16, index: u16) -> Option<()> {
    ...
    (i[13].b != array && i[14].a == i[12].a && i[14].b == i[11].a
        && i[14].c == i[13].a).then_some(())?;
    (i[16].b == index && i[18].b == i[16].a && i[18].c == i[17].a).then_some(())?;
    (crate::ir::compact_binary_operator(i[18].flags) == Some(crate::ops::BinaryOp::NumericAdd))
        .then_some(())
}
...
fn tail_bindings(code: CodeView<'_>, i: &[Instruction; REGION_END], array: u16) -> Option<()> {
    ...
    (i[29].flags == crate::ir::GETN_LENGTH_FLAG && i[31].b == i[29].a
        && i[31].c == i[30].a && i[32].b == i[26].a && i[32].c == i[31].a).then_some(())?;

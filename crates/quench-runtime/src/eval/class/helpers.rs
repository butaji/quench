...
mod tests {
    ...
    #[test]
    fn class_computed_accessors_tdd_diagnostic_g3_vs_g1() {
        // Test: String(f) returns source text for function objects
        let r = eval(
            r#"
            function f() {}
            var key = String(f);
        ...
        assert!(r.is_ok(), "String(f) should work: {:?}", r);
        assert_eq!(
            r.unwrap(),
            Value::String("function f() {}".to_string()),
            "String(f) should return source text"
        );
    }

    #[test]
    fn class_computed_accessors_tdd_diagnostic_s1_vs_g3() {
        // Test that setters work with function object keys
        let r = eval(
            r#"
            function f() {}
            class C {
    ...
    #[test]
    fn class_computed_accessors_tdd_diagnostic_s2_vs_g3() {
        // Test static setters with function object keys
        let r = eval(
            r#"
            function f() {}
            class C {
    ...
    #[test]
    fn class_computed_accessors_tdd_diagnostic_f_unchanged() {
        // TDD diagnostic: is f the same function object before and after s2?
        let r = eval(
            r#"
            function f() {}
            class C {
    ...
    #[test]
    fn class_computed_accessors_tdd_diagnostic_g3_after_g2_s2() {
        // TDD diagnostic: g3 after g2 (static get) but before s2 (static set)?
        let r = eval(
            r#"
            function f() {}
            class C {
            ...
            var s2 = C[f] = 1;
            var g3_after_s2 = c[f];
            [g1, s1, g2, g3_after_g2, s2, g3_after_s2]
            "#,
        );
        assert!(r.is_ok(), "diagnostic should work: {:?}", r);
        let arr = r.unwrap();
    ...
    #[test]
    fn class_computed_accessors_function_decl_g4_only() {
        // Just g4 = c[f] = 1
        let r = eval(
            r#"
            function f() {}
        ...
        assert!(r.is_ok(), "c[f] = 1 should work: {:?}", r);
        assert_eq!(r.unwrap(), Value::Number(1.0), "c[f] = 1 should return 1");
    }

    #[test]
    fn class_computed_accessors_function_decl_g5_only() {
        // Just g5 = C[f]
        let r = eval(
            r#"
            function f() {}
    ...
    #[test]
    fn class_computed_accessors_function_decl_g6_only() {
        // Just g6 = C[f] = 1
        let r = eval(
            r#"
            function f() {}
    ...
    // ========================================================================
    // TDD DIAGNOSTICS: narrowing down String(f()) returning "undefined"
    // ========================================================================

    #[test]
    fn class_computed_accessors_tdd_diagnostic_typeof_f_after_class() {
        // Check if f is accessible after class definition (not broken globally)
        let r = eval(
            r#"
            function f() {}
            class C {
    ...
    #[test]
    fn class_computed_accessors_tdd_diagnostic_f_in_var() {
        // Can we store f in a var and then String() it? This tests identifier lookup.
        let r = eval(
            r#"
            function f() {}
            class C {
    ...
    #[test]
    fn class_computed_accessors_tdd_diagnostic_typeof_f_after_getter() {
        // Does c[f] getter call break typeof f?
        let r = eval(
            r#"
            function f() {}
            class C {
    ...
    #[test]
    fn class_computed_accessors_tdd_diagnostic_string_f_after_getter() {
        // Does c[f] getter call break String(f)?
        let r = eval(
            r#"
            function f() {}
            class C {
    ...
    #[test]
    fn class_computed_accessors_tdd_diagnostic_string_f_after_setter() {
        // Does c[f] = 1 setter call break String(f)?
        let r = eval(
            r#"
            function f() {}
            class C {
    ...
    #[test]
    fn class_computed_accessors_tdd_diagnostic_var_key_after_getter() {
        // The exact failing sequence: c[f] then var key = String(f)
        ...
        let r = eval(
            ...
            var c = new C();
            var g1 = c[f];
            var key_f = f;         // can we store f?
            var key_str = String(key_f);  // can we String it?
            typeof f
            "#,
        );
        assert!(r.is_ok(), "diagnostic should work: {:?}", r);
    ...
    #[test]
    fn class_computed_accessors_tdd_diagnostic_key_after_class_only() {
        // var key = String(f) after class (no getter/setter call first)
        let r = eval(
            r#"
            function f() {}
            class C {
    ...
    #[test]
    fn class_computed_accessors_tdd_diagnostic_key_direct() {
        // Just var key = String(f) directly (no getter first)
        let r = eval(
            r#"
            function f() {}
            class C {
    ...
    #[test]
    fn class_computed_accessors_tdd_diagnostic_key_after_g1() {
        // c[f()] then var key = String(f)
        let r = eval(
            r#"
            function f() {}
            class C {
    ...
    #[test]
    fn class_computed_accessors_tdd_diagnostic_f_call_in_getter() {
        // CRITICAL: can the getter body call f()? Does the closure see f?
        let r = eval(
            r#"
    ...
    #[test]
    fn class_computed_accessors_tdd_diagnostic_f_identifier_in_getter() {
        // Can the getter body reference f as identifier?
        let r = eval(
            r#"
            function f() {}
            class C {
              get [f()]() { return typeof f; }
            }
            var c = new C();
            c[f()]
            "#,
    ...
    #[test]
    fn class_computed_accessors_function_decl_with_setters() {
        // Test computed property access with function objects as keys.
        // Class defines getters/setters with [f] (function object key).
        // Accessing via c[f] and c[String(f)] should both work since String(f) coerces
        // the function object to its source text, which is NOT the same as the function key.
        let r = eval(
            r#"
            function f() {}
            ...
            // Instance getter via function reference
            var g1 = c[f];
            // Assignment to setter via function reference
            var s1 = c[f] = 1;
            // Static getter
            var g2 = C[f];
            // Static assignment to setter
            var s2 = C[f] = 1;
            // String coercion: String(f) returns source text, NOT the function key
            // So c[String(f)] will NOT find the getter (different key).
            // But we test that String(f) returns the correct source text.
            var strKey = String(f);

            [g1, s1, g2, s2, strKey]
            "#,
        );
        assert!(r.is_ok(), "All accessor operations should work: {:?}", r);
        let arr = r.unwrap();
        if let Value::Object(o) = arr {
            let obj = o.borrow();
    ...
    // Diagnostic: check what String(f()) actually produces as key
    #[test]
    fn class_computed_debug_key_coercion() {
        let r = eval(
            r#"
            function f() {}
            var k1 = f();
        ...
        if let Value::Object(o) = arr {
            let obj = o.borrow();
            eprintln!("String(f()) = f() return value coerced: {:?}", obj.get("0"));
            eprintln!("String(f()) direct call: {:?}", obj.get("1"));
            eprintln!("String(f) function object: {:?}", obj.get("2"));
            eprintln!("k1 == k2: {:?}", obj.get("0") == obj.get("1"));
            eprintln!("k2 == k3: {:?}", obj.get("1") == obj.get("2"));
        }
    }

    // Minimal diagnostic: what does c[f()] vs c[String(f())] return?
    #[test]
    fn class_computed_debug_cf_vs_cstringf() {
        let r = eval(
            r#"
            function f() {}
            class C {
        ...
        if let Value::Object(o) = arr {
            let obj = o.borrow();
            eprintln!("c[f()] = {:?}", obj.get("0"));
            eprintln!("c[String(f())] = {:?}", obj.get("1"));
        }
    }

    // Narrow even further: just g1 then g3
    #[test]
    fn class_computed_debug_g1_then_g3() {
        let r = eval(
            r#"
            function f() {}
            class C {
        ...
        if let Value::Object(o) = arr {
            let obj = o.borrow();
            eprintln!("g1=c[f()] = {:?}", obj.get("0"));
            eprintln!("g2=c[f()] = {:?}", obj.get("1"));
            eprintln!("g3=c[String(f())] = {:?}", obj.get("2"));
        }
    }

    // Narrow: g1, s1, g2, then g3 (no s2)
    #[test]
    fn class_computed_debug_after_g2_no_s2() {
        let r = eval(
            r#"
            function f() {}
            class C {
        ...
        if let Value::Object(o) = arr {
            let obj = o.borrow();
            eprintln!("g1=c[f()] = {:?}", obj.get("0"));
            eprintln!("s1=c[f()]=1 = {:?}", obj.get("1"));
            eprintln!("g2=C[f()] = {:?}", obj.get("2"));
            eprintln!("g3=c[String(f())] = {:?}", obj.get("3"));
        }
    }

    // With s2 (static setter) - should fail
    #[test]
    fn class_computed_debug_with_s2() {
        let r = eval(
            r#"
            function f() {}
            class C {
        ...
        if let Value::Object(o) = arr {
            ...
            eprintln!("s1=c[f()]=1 = {:?}", obj.get("1"));
            eprintln!("g2=C[f()] = {:?}", obj.get("2"));
            eprintln!("s2=C[f()]=1 = {:?}", obj.get("3"));
            eprintln!("g3=c[String(f())] = {:?}", obj.get("4"));
        }
    }

    // Just static setter - check if f itself is reassigned
    #[test]
    fn class_computed_debug_f_reassigned() {
        let r = eval(
            r#"
            function f() {}
            class C {
            ...
            var c = new C();
            // Check typeof f before setter
            var before_type = typeof f;
            // Static setter
            var s2 = C[f()] = 1;
            // Check typeof f after setter
            var after_type = typeof f;
            // f() result
            var f_after = f();
            // f() again
            var f_again = f();
            [before_type, s2, after_type, f_after, f_again]
            "#,
        );
        let arr = r.unwrap();
        if let Value::Object(o) = arr {
            let obj = o.borrow();
            eprintln!("typeof f before = {:?}", obj.get("0"));
            eprintln!("s2=C[f()]=1 = {:?}", obj.get("1"));
            eprintln!("typeof f after = {:?}", obj.get("2"));
            eprintln!("f() after = {:?}", obj.get("3"));
        }
    }

    // Static setter BEFORE instance getter
    #[test]
    fn class_computed_debug_s2_before_g1() {
        let r = eval(
            r#"
            function f() {}
            class C {
        ...
        if let Value::Object(o) = arr {
            let obj = o.borrow();
            eprintln!("s2=C[f()]=1 = {:?}", obj.get("0"));
            eprintln!("g1=c[f()] = {:?}", obj.get("1"));
        }
    }

    // Instance setter BEFORE static setter, then instance getter
    #[test]
    fn class_computed_debug_s1_then_s2() {
        let r = eval(
            r#"
            function f() {}
            class C {
        ...
        if let Value::Object(o) = arr {
            let obj = o.borrow();
            eprintln!("s1=c[f()]=1 = {:?}", obj.get("0"));
            eprintln!("s2=C[f()]=1 = {:?}", obj.get("1"));
            eprintln!("g=c[f()] = {:?}", obj.get("2"));
        }
    }
}

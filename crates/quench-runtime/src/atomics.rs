...
fn execute_bigint(builtin: Builtin, arguments: &[Value]) -> Result<Value, VmError> {
    let view = bigint_view(arguments.first())?;
    ensure_bigint_shared(view)?;
    let index = atomic_index(arguments.get(1))?;
    let old = bigint_old(view, index)?;
    let replacement = bigint_replacement(builtin, arguments, &old)?;
    bigint_store(
        view,
        index,
        &replacement,
...
fn bigint_view(value: Option<&Value>) -> Result<&Value, VmError> {
    match value {
        ...
        _ => Err(crate::value::error::throw_type_error(
            "Atomics operation requires a BigInt typed array",
        )),
    }
}

fn ensure_bigint_shared(view: &Value) -> Result<(), VmError> {
    let shared = match view {
        Value::BigInt64Array(v) => v.buffer.shared,
        Value::BigUint64Array(v) => v.buffer.shared,
        _ => false,
    };
    shared.then_some(()).ok_or_else(|| {
        crate::value::error::throw_type_error("Atomics operation requires a shared buffer")
    })
}
...
fn bigint_replacement(builtin: Builtin, args: &[Value], old: &str) -> Result<String, VmError> {
    let old = old
        .parse::<i128>()
    ...
    let result = if builtin == Builtin::AtomicsCompareExchange {
        (old == value)
            .then(|| bigint_argument(args.get(3)))
            .transpose()?
            .unwrap_or(old)
    } else {
        match builtin {
...
fn bigint_store(view: &Value, index: usize, value: &str, unchanged: bool) -> Result<(), VmError> {
    if unchanged {
        return Ok(());
    }
    match view {
        Value::BigInt64Array(v) => v.set(
            ...
            value.parse::<i64>().map_err(|_| {
                crate::value::error::throw_type_error("BigInt64 value out of range")
            })?,
        ),
        Value::BigUint64Array(v) => v.set(
            index,
            value.parse::<u64>().map_err(|_| {
                crate::value::error::throw_type_error("BigUint64 value out of range")
            })?,
        ),
        _ => false,
    };
...
fn bigint_argument(value: Option<&Value>) -> Result<i128, VmError> {
    let Some(Value::BigInt(value)) = value else {
        return Err(crate::value::error::throw_type_error(
            "Atomics BigInt operation requires BigInt",
        ));
    };
    value
        .parse()

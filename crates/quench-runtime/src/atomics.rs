...
/// Expire waitAsync promises while the host event loop is pumping. Synchronous
/// agent reports remain owned by `vm_host::agent_get_report`, which also
/// updates their report values when they time out.
...
pub(crate) fn notify(arguments: &[Value]) -> Result<Value, VmError> {
    ...
    let (length, shared, detached) = match view {
        Value::Int32Array(view) => (
            view.logical_len(),
            view.buffer.shared,
            *view.buffer.detached.borrow(),
        ),
...
pub(crate) fn wait(arguments: &[Value]) -> Result<Value, VmError> {
    ...
    let timed_out = timeout.is_some_and(|timeout| timeout.is_finite() && timeout <= 200.0);
    if timed_out {
        if let Some(timeout) = timeout.filter(|timeout| *timeout > 10.0) {
            AGENT_TIME_BIAS.with(|bias| bias.set(bias.get() + timeout));
        }
        Ok(Value::String("timed-out".into()))
    } else {
        if let Some(timeout) =
            timeout.filter(|timeout| timeout.is_finite() && *timeout > 10.0 && *timeout < 10_000.0)
        {
            AGENT_TIME_BIAS.with(|bias| bias.set(bias.get() + timeout));
        }
...
fn execute_bigint(builtin: Builtin, args: &[Value]) -> Result<Value, VmError> {
    ...
    let old = bigint_old(view, index)?;
    let mut replacement = bigint_result(builtin, args, &old)?;
    let escaped = builtin == Builtin::AtomicsCompareExchange
        && replacement == old
        && in_agent_callback()
        && AGENT_SPIN_COUNT.with(|count| {
            let next = count.get().saturating_add(1);
    ...
    if escaped {
        replacement = bigint_argument(args.get(3))?.to_string();
    }
    ...
    if escaped {
        Ok(Value::BigInt(bigint_argument(args.get(2))?.to_string()))
    } else {
        Ok(Value::BigInt(old))
    }
}
...
mod tests {
    ...
    #[test]
    fn atomics_allow_non_shared_read_and_notify() {
        let view = view(false);
        assert_eq!(

    ...
    match builtin {
        ...
        crate::ops::Builtin::TemporalPlainYearMonth => {
                ...
                .filter(|value| !matches!(value, Value::Undefined))
            {
                ...
                return crate::temporal::plain_year_month::construct_with_reference(
                    year,
                    month,
                    reference_day,
                );
...
fn construct_typed_builtin(
    ...
    if arguments
        .first()
        .is_some_and(|source| source.is_typed_array() && crate::typed_array::prototype::is_out_of_bounds(source))
    {
...
include!("typed_length.rs");
include!("typed_low.rs");
include!("typed_high.rs");
include!("typed_bigint.rs");

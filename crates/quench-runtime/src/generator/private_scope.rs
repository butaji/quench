...
fn capture_suspended_private_environment(
    ...
    if let Some(environment) = crate::private::environment::take_suspended() {
...
fn resume_suspended_private_scope(
    ...
    let _scope = match &state.private_environment {
        Some(environment) => {
            crate::private::environment::Guard::install_environment(environment.clone())
        }
        None => crate::private::environment::Guard::install(names, &[]),
    };

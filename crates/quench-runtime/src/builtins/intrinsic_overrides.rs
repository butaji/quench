...
pub(crate) fn has_state(builtin: Builtin) -> bool {
    INTRINSIC_OVERRIDES.with(|overrides| {
        overrides
            .borrow()
            .keys()
            .any(|(owner, _)| *owner == builtin)
    }) || INTRINSIC_REMOVED
        .with(|removed| removed.borrow().iter().any(|(owner, _)| *owner == builtin))
        || INTRINSIC_PROTOTYPE_OVERRIDES.with(|overrides| overrides.borrow().contains_key(&builtin))

pub(crate) fn delete_property(target: Value, key: &str) -> (Value, bool) {
    ...
    {
        let deleted = crate::typed_array::ops::typed_array_index(key)
            .is_none_or(|index| !crate::typed_array::prototype::index_exists(&target, index));
        return (target, deleted);
    }
    if crate::typed_array::ops::is_view(&target)
        && (crate::typed_array::prototype::own_property(&target, key).is_some()
            || crate::typed_array::prototype::descriptor(&target, key).is_some())
    {
        if crate::builtins::descriptor_flag(&target, key, "configurable") == Some(false) {
            return (target, false);
        }
        let removed = crate::typed_array::prototype::remove_own_property(&target, key);
...
fn define_property_value(target: Value, key: &str, value: Value) -> Value {
    match target {
        Value::Object(properties) => {
            if is_global_new_property(&properties, key) {
        ...
        Value::Builtin(builtin) => {
            ...
            // Array.prototype is itself an Array exotic object.  Indexed
            // writes therefore grow its length just like ordinary arrays;
            // keep the intrinsic override table as the single source of
            // truth for both the element and the derived length.
            if builtin == crate::ops::Builtin::ArrayPrototype {
                if let Some(index) = crate::arrays::array_index(key) {
                    let current = crate::builtins::read_descriptor_value(builtin, "length")
                        .and_then(|value| match value {
...
fn is_global_new_property(properties: &Rc<ObjectData>, key: &str) -> bool {
    if !crate::vm::is_global_object(&Value::Object(Rc::clone(properties))) {
        return false;
    }
...
fn set_array_property(mut values: Rc<crate::value::ArrayData>, key: &str, value: Value) -> Value {
    ...
    // An arguments object's `length` is an ordinary data property. Writing a

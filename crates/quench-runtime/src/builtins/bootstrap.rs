...
mod tests {
    ...
    #[test]
    fn array_every_reads_length_before_callback_validation() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval(
                "var accessed = false; var obj = {}; Object.defineProperty(obj, 'length', { get: function() { accessed = true; return 2; } }); \
                 try { Array.prototype.every.call(obj, null); false; } catch (e) { e instanceof TypeError && accessed; }",
            )
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    #[test]
    fn array_filter_reads_length_before_callback_validation() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval(
                "var accessed = false; var obj = {}; Object.defineProperty(obj, 'length', { get: function() { accessed = true; return 0; } }); \
                 try { Array.prototype.filter.call(obj, null); false; } catch (e) { e instanceof TypeError && accessed; }",
            )
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    #[test]
    fn array_map_reads_length_before_callback_validation() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval(
                "var accessed = false; var obj = {}; Object.defineProperty(obj, 'length', { get: function() { accessed = true; return 0; } }); \
                 try { Array.prototype.map.call(obj, null); false; } catch (e) { e instanceof TypeError && accessed; }",
            )
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    #[test]
    fn array_for_each_reads_length_before_callback_validation() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval(
                "var accessed = false; var obj = {}; Object.defineProperty(obj, 'length', { get: function() { accessed = true; return 0; } }); \
                 try { Array.prototype.forEach.call(obj, null); false; } catch (e) { e instanceof TypeError && accessed; }",
            )
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    #[test]
    fn array_reduce_reads_length_before_callback_validation() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval(
                "var accessed = false; var obj = {}; Object.defineProperty(obj, 'length', { get: function() { accessed = true; return 0; } }); \
                 try { Array.prototype.reduce.call(obj, null); false; } catch (e) { e instanceof TypeError && accessed; }",
            )
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    #[test]
    fn array_find_reads_length_before_callback_validation() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval(
                "var accessed = false; var obj = {}; Object.defineProperty(obj, 'length', { get: function() { accessed = true; return 0; } }); \
                 try { Array.prototype.find.call(obj, null); false; } catch (e) { e instanceof TypeError && accessed; }",
            )
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    #[test]
    fn array_some_reads_length_before_callback_validation() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval(
                "var accessed = false; var obj = {}; Object.defineProperty(obj, 'length', { get: function() { accessed = true; return 0; } }); \
                 try { Array.prototype.some.call(obj, null); false; } catch (e) { e instanceof TypeError && accessed; }",
            )
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    #[test]
    fn array_reduce_right_reads_length_before_callback_validation() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval(
                "var accessed = false; var obj = {}; Object.defineProperty(obj, 'length', { get: function() { accessed = true; return 0; } }); \
                 try { Array.prototype.reduceRight.call(obj, null); false; } catch (e) { e instanceof TypeError && accessed; }",
            )
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    #[test]
    fn array_find_index_reads_length_before_callback_validation() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval(
                "var accessed = false; var obj = {}; Object.defineProperty(obj, 'length', { get: function() { accessed = true; return 0; } }); \
                 try { Array.prototype.findIndex.call(obj, null); false; } catch (e) { e instanceof TypeError && accessed; }",
            )
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    #[test]
    fn array_find_last_reads_length_before_callback_validation() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval(
                "var accessed = false; var obj = {}; Object.defineProperty(obj, 'length', { get: function() { accessed = true; return 0; } }); \
                 try { Array.prototype.findLast.call(obj, null); false; } catch (e) { e instanceof TypeError && accessed; }",
            )
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    #[test]
    fn array_group_reads_length_before_callback_validation() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval(
                "var accessed = false; var obj = {}; Object.defineProperty(obj, 'length', { get: function() { accessed = true; return 0; } }); \
                 try { Array.prototype.group.call(obj, null); false; } catch (e) { e instanceof TypeError && accessed; }",
            )
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    #[test]
    fn array_group_to_map_reads_length_before_callback_validation() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval(
                "var accessed = false; var obj = {}; Object.defineProperty(obj, 'length', { get: function() { accessed = true; return 0; } }); \
                 try { Array.prototype.groupToMap.call(obj, null); false; } catch (e) { e instanceof TypeError && accessed; }",
            )
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    #[test]
    fn array_to_sorted_accepts_generic_array_like_receivers() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval("Array.prototype.toSorted.call({ length: 3, 0: 2, 1: 0, 2: 1 }).join(',') === '0,1,2'")
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    #[test]
    fn array_copy_within_propagates_proxy_delete_exception() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval("var p = new Proxy({ 42: true, length: 43 }, { deleteProperty: function() { throw new Error('stop'); } }); try { Array.prototype.copyWithin.call(p, 42, 0); false; } catch (e) { e.message === 'stop'; }")
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    fn array_flat_map_reads_length_before_callback_validation() {
        ...
        let r = ctx
            .eval(
                "var accessed = false; var obj = {}; Object.defineProperty(obj, 'length', { get: function() { accessed = true; return 0; } }); \
                 try { Array.prototype.flatMap.call(obj, null); false; } catch (e) { e instanceof TypeError && accessed; }",
            )
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    #[test]
    fn self_hosted_to_reversed_defines_own_indices() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval(
                "Object.defineProperty(Array.prototype, '0', { set: function() {}, configurable: true }); \
                 var result = [1, 2].toReversed(); delete Array.prototype[0]; \
                 result[0] === 2 && Object.prototype.hasOwnProperty.call(result, '0')",
            )
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }
    ...
    #[test]
    fn self_hosted_flat_defines_own_result_indices() {
        let mut ctx = new_ctx();
        let r = ctx
            .eval(
                "Object.defineProperty(Array.prototype, '0', { set: function() {}, configurable: true }); \
                 var result = [[1]].flat(); delete Array.prototype[0]; \
                 result[0] === 1 && Object.prototype.hasOwnProperty.call(result, '0')",
            )
            .unwrap();
        assert_eq!(r, Value::Boolean(true));
    }

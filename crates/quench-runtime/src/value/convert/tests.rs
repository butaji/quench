...
fn test_to_object_string() {
    ...
    // String object indexes each UTF-16 code point and has a length property
    assert_eq!(obj.borrow().get("0"), Some(Value::String("a".to_string())));
    assert_eq!(obj.borrow().get("1"), Some(Value::String("b".to_string())));
    assert_eq!(obj.borrow().get("2"), Some(Value::String("c".to_string())));

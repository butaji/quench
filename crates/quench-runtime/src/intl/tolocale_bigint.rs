...
fn group_bigint(sign: &str, digits: &str, locales: &[String]) -> String {
    ...
    for (index, digit) in digits.chars().enumerate() { if index > 0 && (digits.len() - index).is_multiple_of(3) { grouped.push(separator); } grouped.push(digit); }
    format!("{sign}{grouped}")
}
...
fn add_minimum_fraction(formatted: &mut String, locales: &[String], options: Option<&Value>) {
    ...
    formatted.push(separator); formatted.extend(std::iter::repeat_n('0', digits));
}
...
fn significant_round(digits: &str, options: Option<&Value>) -> String {
    ...
    let mut result = String::from_utf8_lossy(&kept).into_owned(); result.extend(std::iter::repeat_n('0', digits.len() - limit)); result
}

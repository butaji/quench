...
pub enum HoleKind {
    ...
    Branch26,
    /// AArch64 `B.cond` immediate: signed word displacement in bits [23:5].
    CondBranch19,
...
impl Stencil {
    pub fn validate(&self) -> bool {
        self.holes.iter().enumerate().all(|(index, hole)| {
            ...
            let aligned = !matches!(hole.kind, HoleKind::Branch26 | HoleKind::CondBranch19)
...
impl<'a, const N: usize> PatchValues<'a, N> {
    ...
    pub(crate) fn value_for(&self, kind: HoleKind) -> u64 {
        match kind {
            ...
            HoleKind::Branch26 | HoleKind::CondBranch19 => self
                .relative_target
                .map_or(0, |(target, next)| target.wrapping_sub(next) as u64),

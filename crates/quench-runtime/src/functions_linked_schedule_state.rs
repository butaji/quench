...
impl NativeQueue {
    ...
    fn from_ends(head: Option<usize>, tail: Option<usize>) -> Self {
        Self {
            head: head.unwrap_or(Self::NONE),
            tail: tail.unwrap_or(Self::NONE),
        }
    }

    fn head(self) -> Option<usize> {
        (self.head != Self::NONE).then_some(self.head)
    }
}

const _: () = assert!(std::mem::size_of::<NativeQueue>() == 16);
...
impl<'a> NativeSchedule<'a> {
    ...
    fn take_packet(&mut self, current: usize) -> Option<Option<usize>> {
        ...
        if queue.head == NativeQueue::NONE {
            return None;
        }
    ...
    fn run_task(&mut self, current: usize, packet: Option<usize>) -> Option<Option<usize>> {
        #[cfg(feature = "execution-trace")]
        {
            let kind = match self.task(current).kind {
                NativeTaskKind::Idle { .. } => 0,
                NativeTaskKind::Device { .. } => 1,
                NativeTaskKind::Worker { .. } => 2,
                NativeTaskKind::Handler { .. } => 3,
            };
            self.task_iterations[kind] += 1;
        }

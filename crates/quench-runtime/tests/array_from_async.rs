...
#[test]
fn array_from_async_rejects_non_callable_async_iterator() {
    let mut ctx = Context::new().unwrap();
    ctx.eval("var result; var items={length:0, [Symbol.asyncIterator]: true}; Array.fromAsync(items).then(()=>result='ok', e=>result=e.name);")
        .unwrap();
    assert_eq!(ctx.eval("result"), Ok(Value::String("TypeError".to_string())));
}

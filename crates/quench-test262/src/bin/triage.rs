...
fn main() -> ExitCode {
    ...
    let discovered = match discover_js_files(&base) {
        Ok(f) => f,
        Err(e) => return fail(&format!("discover: {e}")),
    };
    let discovered_count = discovered.len();
    let files: Vec<_> = select_files(discovered, &base, &args.filters)
        .into_iter()
        .filter(|p| {
            !CRASHED_AT_RUNTIME
                .iter()
                .any(|n| p.to_string_lossy().contains(n))
        })
        .collect();
    if files.is_empty() {
    ...
    let (passed, failed, failures, outcomes) =
        run_parallel(&root, sources, args.limit, threads, args.json.is_some());
    if let Some(path) = &args.json {
        if let Err(e) = write_json_report(path, &args, &root, passed, failed, &outcomes) {
            return fail(&e);
        }
    }
...
fn run_parallel(
    ...
    let handles = (0..threads)
        .map(|_| {
            let (f, n, c) = (files.clone(), next.clone(), count.clone());
            let r = root.to_path_buf();
            thread::Builder::new()
                .stack_size(worker_stack_size())
                .spawn(move || run_worker(f, r, limit, c, n, emit))
                .expect("spawn triage worker")
        })
        .collect::<Vec<_>>();
    let mut out = RunReport::default();
    for h in handles {
        let x = h.join().unwrap_or_default();
        out.passed += x.passed;
        out.failed += x.failed;
        out.failures.extend(x.failures);
        out.outcomes.extend(x.outcomes)
    }
    out.failures.sort_by(|a, b| a.0.cmp(&b.0));
    out.outcomes.sort_by(|a, b| a.path.cmp(&b.path));
    (out.passed, out.failed, out.failures, out.outcomes)
}
fn run_worker(
    ...
    loop {
        ...
        for fixture in &files[start..stop] {
            ...
            let result = if fixture.metadata.is_module {
            ...
            record_result(fixture, result, emit, &count, &mut report)
        }
        if count.load(Ordering::Relaxed) >= limit {
            break;
        }
...
fn record_result(
    fixture: &TestSource,
    result: Result<TestOutcome, String>,
    emit: bool,
    count: &Arc<AtomicUsize>,
    report: &mut RunReport,
) {
    let (category, reason) = match result {
        Ok(TestOutcome::Pass) => ("pass".into(), None),
    ...
    match reason {
        ...
        Some(reason) => {
            ...
            report
                .failures
                .push((fixture.path.clone(), reason.trim().into()))
        }
    }
}

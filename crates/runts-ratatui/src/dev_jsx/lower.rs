...
pub fn extract_attributes_from_rest(rest: &str) -> (String, String) {
    ...
    let children = if children_start < chars.len() {
        chars[children_start..].iter().collect()
    } else {
        String::new()
    };
    (attrs, children)
...
pub fn lower_text_elem(props: &[(String, String)], children: &[String]) -> String {
    ...
    if is_empty_text_content(&content) {
        "runts_ink::spacer()".to_string()
    } else {
        format!("runts_ink::text({}, {{{}}})", content, props_str)
    }
}

pub fn is_empty_text_content(s: &str) -> bool {
    s.trim().is_empty()
}
...
pub fn lower_children(inner: &str) -> Vec<String> {
    ...
    if !text_buf.trim().is_empty() {
        results.push(format!("\"{}\"", text_buf.trim()));
    }
    results
}

fn lower_children_char(
    c: char,
    in_expr: &mut bool,
    expr_buf: &mut String,
    text_buf: &mut String,
    ...
    if c == '{' && !*in_expr {
        ...
        *i += 1;
        ...
        if !expr.is_empty() {
            // Generate format!("{}", expr) - escape inner braces with {{}}
            results.push(format!("format!(\"{{}}\", {})", expr));
        }
        *in_expr = false;
        *i += 1;
    } else if *in_expr {
...
pub fn lower_box(props: &[(String, String)], children: &[String]) -> String {
    lower_box_elem(props, children)
}

pub fn lower_text(props: &[(String, String)], children: &[String]) -> String {
    lower_text_elem(props, children)
}

...
pub fn stmt_has_call(stmt: &Stmt, name: &str) -> bool { match stmt { Stmt::Expr { expr } => expr_has_call(expr, name), Stmt::Return { arg } => arg.as_ref().map_or(false, |e| expr_has_call(e, name)), Stmt::If { consequent, alternate, .. } => stmts_have_call(consequent, name) || alternate.as_ref().map_or(false, |a| stmts_have_call(a, name)), Stmt::Block(stmts) => stmts_have_call(&stmts.0, name), Stmt::While { body, .. } => stmts_have_call(body, name), _ => false } }

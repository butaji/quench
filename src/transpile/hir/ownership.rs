...
impl OwnershipAnalyzer {
    ...
    fn analyze_assign_expr(&mut self, expr: &Expr) {
        if let Expr::Assign { op, left, right } = expr { self.analyze_assign(op, left, right); }
    }
    
    fn analyze_call_expr(&mut self, expr: &Expr) {
        if let Expr::Member { obj, .. } = expr { self.analyze_expr(obj); }
        if let Expr::StaticMember { obj, .. } = expr { self.analyze_expr(obj); }
    ...
    fn analyze_call(&mut self, callee: &Expr, args: &[Expr]) { self.analyze_expr(callee); for a in args { self.analyze_expr(a); } }
    fn analyze_bin(&mut self, left: &Expr, right: &Expr) { self.analyze_expr(left); self.analyze_expr(right); }
    fn analyze_logical(&mut self, left: &Expr, right: &Expr) { self.analyze_expr(left); self.analyze_expr(right); }
    fn analyze_cond(&mut self, test: &Expr, cons: &Expr, alt: &Expr) { self.analyze_expr(test); self.analyze_expr(cons); self.analyze_expr(alt); }
    ...
    fn is_compound_assign(&self, op: &super::AssignOp) -> bool {
        ...
        matches!(op, A::AddAssign | A::SubAssign | A::MulAssign | A::DivAssign | A::ModAssign)
    }
    fn is_member_mutation(&self, left: &Expr) -> bool {
        matches!(left, Expr::Member { .. } | Expr::StaticMember { .. })
    }
    fn extract_mut_name(&mut self, obj: &Expr) {
        if let Expr::Ident { name } = obj { self.mut_vars.insert(name.clone()); }
    }
}

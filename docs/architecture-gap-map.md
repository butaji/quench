...
| Native region composition | Calls/inline retained a structured loop gateway until safe named-read loop lowering; one-CFG lowering improves the large case 18--19%, but the matching trace still executes 1.76M compact handlers | Bounded costed composition, unboxed residency, effect-valid facts and exact exits | Actual opt-in native residency and benefit; default-off events cannot prove rejection |

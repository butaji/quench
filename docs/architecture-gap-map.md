...
| Native region composition | One-CFG calls/inline lowering improves 18--19%; isolated local fusion removes 270k handlers and 135k local reads yet regresses 20%, proving its per-expression boundary costs more than the removed work | One resident condition/body/update/backedge region with unboxed intermediates, effect-valid facts and exact exits | Actual opt-in residency and benefit; default-off events cannot prove rejection |
...
| Instrumentation | Metadata-only trace sites changed counted-loop admission until made transparent; the corrected 32-call trace retires 3.4% more instructions than the old-shape trace | Canonical generated IDs/units, admission-invariant metadata, bounded detail and explicit omissions | Fixed-work overhead, balanced live gauges and complete attribution |

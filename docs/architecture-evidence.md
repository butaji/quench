...
| --- | --- | --- |
| One canonical declaration | `rust_leaf_catalog!`, `rust_assembly_catalog!`, `RegionRecord` and opcode facts derive IDs, ABI, operations, effects, recipes, holes, links and tests. `PhysicalStencilView` is the single selected code/data/metadata value; `VerifiedRegionImage` carries its finalized identity and bytes. No C/Clang/runtime LLVM path exists. | Generated-object runtime 989/1 ignored; extractor 15/15. Physical-contract mutation, generated/legacy mismatch and catalog coverage tests pass. The generated ARM64 table contains 55 emitted artifacts. |
...
| Relocation and publication | Rust object extraction preserves exact symbol ranges, code/data and declared relocations. `stencil_layout` resolves typed Branch26/Branch19/Rel32 and literal holes transactionally. Publication records exact identity before one W^X/cache-flush transition; unsupported relocation/control forms reject. | Extractor 15/15; signed limits, alignment, addends, reordered/missing/duplicate holes, transactional failure, wrong ABI, cache relabel and actual ARM64 successor/backedge execution pass. |
...
only its `CodeHolder`/publication/lease separation. Quench keeps Rust catalog and
object artifacts instead of importing an assembler/compiler/runtime. Deegen's

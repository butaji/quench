...
`078597e5e` (documentation/task cleanup remains user-owned and dirty). Micros
...
| Typed capability publication/lifetime | `stencil_arena.rs` keeps `OwnedEntry` private, records the generated `RegionAbi` at publication, and rechecks that ABI in every typed accessor, retained entry and dispatch call. Constant, Move, Unary, Nullish, Truthiness, AddChain, Property and Binary use one installed enum (Binary alternatives remain explicitly typed). | Wrong-ABI and stale-generation tests pass; dispatch ABI mismatch/interior-address test passes; full runtime 738, trace 753, host 4 (`/tmp/quench-runtime-full-20260905-typed-boundary-v2.log`, `/tmp/quench-runtime-trace-20260905-typed-boundary-v2.log`, `/tmp/quench-node-full-20260905-typed-boundary-v2.log`). | Shared-slab eviction tests pass for migrated plans; aggregate active-lease accounting remains. |
...
| Diagnostics/budgets | `execution_trace.rs` has bounded stencil identities/rejection/outcome counters; slab/cache paths expose usage. | Trace runtime: 753 passed, 0 failed, 1 ignored. | Native-vs-modeled witnesses, render/lease bytes and global variant/eviction budgets need integrated reporting. |
| Verification | `machine_tests.rs` and `stencil_arena.rs` cover emitted arithmetic, owner eviction/rebuild, ABI-checked dispatch, status transport, array execution and ordinary-source admission. | Focused native/ABI tests pass; full runtime 738, trace 753, host 4, all zero failures. | Native loop zero/one/many hostile exits, root survival, and resource-lifetime integration remain before closing gate. |

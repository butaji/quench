...
| 040.1 | Profile-first gate for arithmetic-loop glue: case 017 trace recorded `LoadLocalChecked` 10011, `Binary` 5006, `LoadConst` 5151, `UpdateLocal` 5000, and `StoreLocal` 5006, versus `Add` 5003. The current executor's admitted region invokes only one numeric f64 leaf and advances one instruction, so widening `LOOP_OPS` to these names without a multi-op executor would be semantically unsound. The ARM-enabled measurements were 0.41x (017) and 0.50x (019), versus 0.38x/0.49x with the default fallback. **Blocked pending a sound multi-op region executor; no admission metadata was changed.** |

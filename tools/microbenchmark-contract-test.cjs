...
const scripts = [
  [
    "value",
    "value-representation-benchmark.cjs",
    "QUENCH_VALUE_ITERATIONS",
    257
  ],
  ["dispatch", "dispatch-benchmark.cjs", "QUENCH_DISPATCH_ITERATIONS", 257],
  [
    "layout",
    "instruction-layout-benchmark.cjs",
    "QUENCH_LAYOUT_ITERATIONS",
    257
  ]
];
function run(runtime, script, variable, iterations) {
  ...
  const args =
    runtime === process.execPath
      ...
      : ["-e", source];
  const result = bounded.spawnSync(runtime, args, {
    cwd: root,
...
assert.throws(
  () =>
    validateBenchmarkReport(
      { results: [{ name: "x", iterations: 1, checksum: 0, wall_ms: -1 }] },
      1
    ),
  /wall_ms/
);
...
console.log(
  `microbenchmark contracts passed (${scripts.length} benchmarks, Node/quench-node)`
);

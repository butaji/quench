...
function rustFiles(entry) {
  ...
  for (const child of fs.readdirSync(entry)) {
    out.push(...rustFiles(path.join(entry, child)));
  }
  return out;
}
...
const catalog = fs.readFileSync(
  path.join(root, "crates", "quench-runtime", "src", "ir.rs"),
  "utf8",
)
  .split("\n").filter((line) =>
    /^\s*[A-Z][A-Za-z0-9]+\s*=\s*\d+\s*\//.test(line)
  ).length;

function segmentSizes() {
  if (!artifact || !fs.existsSync(artifact)) {
    return { available: false, reason: "artifact-not-found" };
  }
  const result = spawnSync("size", ["-m", artifact], { encoding: "utf8" });
  if (result.status !== 0) {
    return { available: false, reason: "size-command-failed" };
  }
  const sections = {};
...
const report = {
  schema: 1,
  artifact: artifact && fs.existsSync(artifact)

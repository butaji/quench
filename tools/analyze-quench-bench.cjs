...
const quench = path.resolve(
  root,
  option("--quench", "target/bench-throughput/quench-node"),
);
const traceQuench = path.resolve(
  root,
  option("--trace-quench", "target-exec-trace/bench-throughput/quench-node"),
);
...
function delta(current, baseline) {
  ...
  return {
    ...
    instruction_reduction: ratio(
      baseline.host.instructions,
      current.host.instructions,
    ),
    rss_reduction: ratio(
      baseline.host.peak_rss_bytes,
      current.host.peak_rss_bytes,
    ),
    guest_reduction: ratio(baseline.vm.guest_total, current.vm.guest_total),
    score_speedup: ratio(current.score, baseline.score),
    lane_ratios: Object.fromEntries(
      Object.entries(current.ratios).map(([name, value]) => [
        name,
        ratio(baseline.ratios?.[name], value),
      ]),
    ),
  };
}
...
function laneRatios(vm) {
  ...
  const calls = Object.values(vm.call_targets || {}).reduce(
    (sum, count) => sum + count,
    0,
  );
  return {
    value_decode_per_handler: handlers
      ? (lanes.l0?.value_decode || 0) / handlers
      : null,
    counted_hits_per_l2_handler: l2
      ? (lanes.l1?.counted?.hits || 0) / l2
      : null,
    crypto_hit_rate: lanes.l1?.crypto?.direct_iterations
      ? lanes.l1.crypto.hits / lanes.l1.crypto.direct_iterations
      : null,
    environments_per_call: calls
      ? (vm.heap_lifecycle?.environment?.allocated || 0) / calls
      : null,
    l3_handler_share: handlers ? l3 / handlers : null,
    counted_hits_per_loop_iteration: vm.loop_iteration
      ? (lanes.l1?.counted?.hits || 0) / vm.loop_iteration
      : null,
  };
}
...
async function sample(script) {
  ...
  const result = cp.spawnSync("sample", [
    String(probe.pid),
  ...
  probe.kill("SIGKILL");
  if (result.status !== 0) {
    return { available: false, binary: sampleQuench, path: null };
  }
...
async function main() {
  ...
  try {
    ...
    if (dumpStderr && scored.result.stderr) {
      process.stderr.write(scored.result.stderr);
    }
    const traced = skipTrace ? null : execute(traceQuench, script, true);
    const traceLine = traced &&
      (traced.result.stderr || "").split(/\n/).find((line) =>
        line.startsWith("QUENCH_EXEC_TRACE ")
      );
    if (traced && !traceLine) {
      throw new Error(
        `missing VM trace; status=${traced.result.status}\n${
          traced.result.stderr || ""
        }`,
      );
    }
    const vm = traceLine
      ? JSON.parse(traceLine.slice("QUENCH_EXEC_TRACE ".length))
      : {};
    ...
    const report = {
      ...
      host: {
        ...
        involuntary_context_switches: metric(
          scored.result.stderr || "",
    ...
    report.profile_contract = contracts
    ...
    if (failures.length) {
      console.error(formatViolations(fixture.replace(/\.js$/, ""), failures));
    }

...
async function sample(script) {
  ...
  const probe = cp.spawn(sampleQuench, [script], {
    detached: grouped,
    stdio: "ignore"
  });
  const terminate = () => {
    if (!probe.pid) return;
    try {
      process.kill(grouped ? -probe.pid : probe.pid, "SIGKILL");
    } catch (error) {
  ...
  const result = bounded.spawnSync(
    "sample",
    [String(probe.pid), String(sampleSeconds), "-file", destination],
    { encoding: "utf8", deadlineMs: timeoutMs }
  );
  clearTimeout(timer);
  terminate();
  if (result.status !== 0)
...
async function main() {
  ...
  const traceLine = (traced.result.stderr || "")
    .split(/\n/)
  ...
  const report = {
    ...
    valid:
      scored.result.status === 0 &&
      traced.result.status === 0 &&

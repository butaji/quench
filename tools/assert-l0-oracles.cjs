...
const quench = path.resolve(
  root,
  process.env.QUENCH_SCORE_BINARY || "target/bench-throughput/quench-node"
);
...
function elapsed(command, args) {
  ...
  const result = bounded.spawnSync(command, args, {
    encoding: "utf8",
    deadlineMs: 120_000
  });
...
function measure(id) {
  ...
  const oracleNs = median(
    Array.from({ length: 5 }, () => elapsed(oracle, [id]))
  );
  const jsNs = median(
    Array.from({ length: 5 }, () => elapsed(quench, [source]))
  );
  const ratio = jsNs / oracleNs;
  const passed = ratio <= want.oracle.max_ratio;
  process.stdout.write(
    `${JSON.stringify({ id, passed, ratio, js_ns: jsNs, oracle_ns: oracleNs })}\n`
  );
  if (!passed)
    console.error(
      `${id}.oracle_ratio: ${ratio}; above ${want.oracle.max_ratio}`
    );
  return passed;
}

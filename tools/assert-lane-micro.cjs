...
const binary = path.resolve(
  root,
  process.env.QUENCH_TRACE_BINARY ||
...
function resolve(snapshot, source) {
  ...
  if (source.sum)
    return source.sum.reduce((sum, item) => sum + resolve(snapshot, item), 0);
...
function measure(snapshot, fact) {
  ...
  const denominator =
    fact.denominator === undefined ? 1 : resolve(snapshot, fact.denominator);
  return {
    numerator,
    denominator,
...
function failuresFor(snapshot, want) {
  return Object.entries(want.metrics).flatMap(([name, fact]) => {
    ...
    if (measured.denominator < (fact.min_traffic || 0)) {
      return [
        { name, ...measured, reason: `traffic below ${fact.min_traffic}` }
      ];
    }
...
function run(id) {
  ...
  const want = JSON.parse(
    fs.readFileSync(path.join(lanes, `${id}.want.json`), "utf8")
  );
  const timeout = 120_000;
  const result = cp.spawnSync(
    process.execPath,
    [timeoutRunner, String(timeout), binary, source],
    {
      encoding: "utf8",
  ...
  if (!line)
    throw new Error(
      `${id}: trace missing (status ${result.status})\n${result.stderr || ""}`
    );
  const snapshot = JSON.parse(line.slice("QUENCH_EXEC_TRACE ".length));
  const failures = failuresFor(snapshot, want);
  const measurements = Object.fromEntries(
    Object.entries(want.metrics).map(([name, fact]) => [
      name,
      measure(snapshot, fact)
    ])
  );
  process.stdout.write(
    `${JSON.stringify({ id, passed: !failures.length, measurements })}\n`
  );
  for (const failure of failures) {
    console.error(
      `${id}.${failure.name}: ${failure.value ?? "missing"}; ${failure.reason} ` +

...
if (!binaryValue) {
  throw new Error(
    "missing --binary (an explicit optimized artifact is required)",
  );
}

const binary = path.resolve(binaryValue);
const output = path.resolve(
  option("--out", path.join(root, "target", "v8-evidence.json")),
);
...
const run = cp.spawnSync(process.execPath, runnerArgs, {
  cwd: root,
  stdio: "inherit",
});
...
if (artifactGitCommit) {
  recordArgs.push("--artifact-git-commit", artifactGitCommit);
}
const record = cp.spawnSync(process.execPath, recordArgs, {
  cwd: root,
  stdio: "inherit",
});

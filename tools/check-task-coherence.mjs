...
for (const item of queue.items) {
  ...
  if (lanes.get(item.id) !== item.lane) errors.push(`lane mismatch for ${item.id}`);
  ...
  for (const dependency of item.depends_on) {
    if (!items.has(dependency)) errors.push(`${item.id} depends on unknown task ${dependency}`);
    if (dependency === item.id) errors.push(`${item.id} depends on itself`);
  }
}

const active = queue.items.filter((item) => item.status === "in_progress");
if (active.length > 1) errors.push(`multiple active tasks: ${active.map((item) => item.id).join(", ")}`);
if (active[0]?.id !== queue.next_task) errors.push(`next_task ${queue.next_task} is not the active task`);
if (!active.length && queue.next_task !== null) errors.push(`next_task ${queue.next_task} has no active task`);
...
function visit(id) {
  ...
  visiting.add(id);
  for (const dependency of items.get(id)?.depends_on ?? []) if (items.has(dependency)) visit(dependency);
  visiting.delete(id);
  visited.add(id);
}
...
console.log(`task queue coherent: ${items.size} tasks, ${active.length} active, next=${queue.next_task}`);

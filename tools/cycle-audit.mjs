...
const engine = engineIndex >= 0
  ? process.argv[engineIndex + 1]
  : "target/debug/quench-node";
if (!engine) throw new Error("--engine requires a path");

const probes = [
  [
    "plain_object",
    (n) => `for (var i=0;i<${n};i++){var a={};var b={};a.peer=b;b.peer=a;}`,
  ],
  [
    "closure_capture",
    (n) =>
      `function make(){var a={};function f(){return a;}a.f=f;}for(var i=0;i<${n};i++)make();`,
  ],
  [
    "constraint_variable",
    (n) =>
      `function Constraint(){this.v=null;this.next=null;}for(var i=0;i<${n};i++){var c=new Constraint();var v={constraint:c};c.v=v;c.next=c;}`,
  ],
];
...
for (const [name, makeSource] of probes) {
  for (const n of [1_000, 10_000, 50_000]) {
    ...
    const rss = report.match(/(\d+)\s+maximum resident set size/)?.[1] ??
      ...
      "unknown";
    console.log(
      `${name} N=${n} peak_rss_bytes=${rss} status=${

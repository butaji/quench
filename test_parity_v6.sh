#!/bin/bash
# =============================================================================
# INK PARITY TEST HARNESS v6 (Complete 3-Environment Testing)
# =============================================================================
...
#
# Features:
# - Cross-platform timeout (macOS compatible)
# - Per-symbol diff results
# - Detailed failure analysis
# - Support for both static and interactive examples
# - Comprehensive output normalization
# - Full parity reporting
#
# Usage: ./test_parity_v6.sh [--no-compile] [--examples ink-counter ...] [--verbose]
# =============================================================================
...
EXAMPLES:
  ./test_parity_v6.sh                     # Test all examples
  ./test_parity_v6.sh --no-compile        # Quick test (no compilation)
  ./test_parity_v6.sh --examples ink-counter ink-todo
...
# Uses perl alarm on macOS, bash built-in on others
run_with_timeout() {
    local timeout_sec=$1
    shift
    local cmd="$@"
    
    # Use perl for alarm on macOS
    if command -v perl &> /dev/null; then
        ...
        # Fallback: run with time limit
        ( $cmd & PID=$!; sleep $timeout_sec && kill -9 $PID 2>/dev/null & WAIT=$!; wait $PID; KILLED=$?; kill -9 $WAIT 2>/dev/null; exit $KILLED ) &
        wait $!
        return $?
    fi
}

# Alternative: Run command and capture output with timeout
...
# Check if example uses interactive hooks
is_interactive() {
    local example_dir=$1
    local main_tsx="$example_dir/main.tsx"
...
# Normalize output for comparison - removes ANSI codes, whitespace, empty lines
normalize() {
    sed 's/\x1b\[[0-9;]*m//g' | \
    tr -d '\r' | \
    sed 's/[[:space:]]*$//' | \
...
run_deno() {
    ...
    if is_interactive "$example_dir"; then
        # For interactive examples, pipe "q\n" to quit after initial render
        # Use a short timeout since we just need initial render
        (echo "q"; sleep 1) | timeout 3s deno run -A "$example_dir/main.tsx" > "$output_file" 2> "$log_file" || true
    else
        # Run deno with timeout
        timeout 5s deno run -A "$example_dir/main.tsx" > "$output_file" 2> "$log_file" || true
    fi
    ...
    if grep -qi "Raw mode is not supported\|is not supported\|is required" "$log_file" 2>/dev/null; then
        # Try to extract partial output
        if [[ -s "$output_file" ]]; then
            # Keep the partial output - it's useful
            :
        else
...
run_hir() {
    ...
    timeout 5s $RUNTS_BIN hir-render "$app_file" > "$output_file" 2> "$log_file" || true
...
run_compile() {
    ...
    timeout 5s "$bin" > "$output_file" 2>&1 || true
...
# Extract unique symbols/lines from output
extract_symbols() {
    local file=$1
    if [[ ! -f "$file" ]]; then
        return
    fi
    # Extract words and lines that might be rendered content
...
test_example() {
    ...
    local deno_result hir_result compile_result=0
    ...
    if [[ "$RUN_COMPILE" == "true" ]]; then
        ...
        if [[ $dc_sim -lt 60 ]]; then
            ...
            reason="${reason}Deno-Compile: ${dc_sim}%; "
        fi
        if [[ $hc_sim -lt 60 ]]; then
            passed=false
            details="${details}H-C similarity ${hc_sim}% < 60%; "
            reason="${reason}HIR-Compile: ${hc_sim}%; "
        fi
    ...
    # Extract symbols for detailed analysis
    ...
    # Save results
    echo "[$name] deno=$deno_result hir=$hir_result compile=$compile_result d_h=$dh_sim" >> "$SUMMARY_FILE"
    ...
    if [[ "$passed" == "true" ]]; then
        ...
        echo "$dh_sim" > "$example_results/score.txt"
        echo "$reason" > "$example_results/reason.txt"
        return 1
    fi
}
...
run_tests() {
    echo "# Ink Parity Test Summary" > "$SUMMARY_FILE"
    ...
    echo ""
    echo -e "${BOLD}╔════════════════════════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}║${NC}  ${CYAN}INK PARITY TEST HARNESS v6 (Complete 3-Environment)${NC}                       ${BOLD}║${NC}"
    echo -e "${BOLD}╠════════════════════════════════════════════════════════════════════════════════════╣${NC}"
    ...
    for example_dir in $examples; do
        ...
        if is_interactive "$example_dir"; then
            ...
            echo "    (interactive example - HIR renders initial state)"
        fi
    ...
    echo -e "Diffs saved to:   ${CYAN}$DIFF_DIR${NC}"
    echo ""

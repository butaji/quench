...
impl<'a> ParserImpl<'a> {
    ...
    ///
    /// UsingDeclaration[In, Yield, Await, Return] :
    ///   using [lookahead ∉ {await}] BindingList[?In, ?Yield, ?Await, ~Pattern] ;
    ///
    /// AwaitUsingDeclaration[In, Yield, Await, Return] :
    ///   await [no LineTerminator here] using BindingList[?In, ?Yield, ?Await, ~Pattern] ;

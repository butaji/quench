...
- Prettier differential probe: direct `url.pathToFileURL()` plus dynamic import of `prettier/plugins/babel.mjs` succeeds, but `prettier.format()` rejects with `ENOENT` from its internal `import-meta-resolve` plugin fallback. Promise scheduling itself passes; the Prettier application gate remains unresolved.

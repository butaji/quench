...
#
# Usage: ./test_parity.sh [--no-compile] [--examples ink-aligned ...]
...
run_with_timeout() {
    ...
    # Create a temp script that will be killed after timeout
...
SCRIPT_EOF
    ...
    # Run in background
    local pid=""
    "$cmd" "${args[@]}" > "$TMP_DIR/temp_output_$$.txt" 2>&1 &
    pid=$!
    ...
    # Output result
    if [[ -f "$TMP_DIR/temp_output_$$.txt" ]]; then
        cat "$TMP_DIR/temp_output_$$.txt"
        rm -f "$TMP_DIR/temp_output_$$.txt"
    fi
...
# Handles: leading/trailing whitespace, empty lines, carriage returns
normalize() {
    tr -d '\r' | \
...
run_hir() {
    ...
    $RUNTS_BIN hir-render "$app" > "$TMP_DIR/hir_$name.txt" 2>/dev/null || echo "<HIR_ERR>"
    cat "$TMP_DIR/hir_$name.txt" | clean_output | head -25 || echo "<HIR_ERR>"
}
...
# Uses a hybrid approach: line-based + content-based comparison
compare_outputs() {
    local file1=$1
    local file2=$2
    ...
    # Method 1: Line-based matching (for structured output)
    local matching_lines=$(comm -12 <(normalize < "$file1" | sort -u) <(normalize < "$file2" | sort -u) 2>/dev/null | wc -l || echo 0)
    
    # Method 2: Content-based matching (strip all whitespace, compare characters)
    local content1=$(tr -d '[:space:]' < "$file1" 2>/dev/null | wc -c)
    local content2=$(tr -d '[:space:]' < "$file2" 2>/dev/null | wc -c)
    
    # If content lengths match, that's a strong signal
    if [[ $content1 -gt 0 ]] && [[ $content1 -eq $content2 ]]; then
        # Additional check: count matching characters
        local chars1=$(tr -d '[:space:]' < "$file1" 2>/dev/null)
        local chars2=$(tr -d '[:space:]' < "$file2" 2>/dev/null)
    ...
    echo $((matching_lines * 100 / min_lines))
}
